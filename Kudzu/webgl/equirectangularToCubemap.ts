...
export async function equirectangularToCubemap(image: TexImageSource, size: number, onProgress?: progressCallback): Promise<CanvasTypes> {
    ...
    return await usingAsync(new Texture(ctx3d.gl, image), async (texture) =>
        await usingAsync(new MaterialEquirectangular(ctx3d.gl, texture), async (material) =>
            await usingAsync(new Geometry(ctx3d.gl, invCube.verts, invCube.indices, ctx3d.gl.TRIANGLES), async (geom) => {
