...
const warpToEvt = Object.assign(new Event("warpTo"), {
    id: null
});
...
export class UserDirectoryForm extends FormDialog {
    constructor() {
        ...
        this.entry = InputText(id("chatEntry"), disabled, onFocus(() => this.chatFocused = true), onFocus(onFocusChanged), onBlur(() => this.chatFocused = false), onBlur(onFocusChanged), onKeyPress((evt) => {
            if (evt.key === "Enter") {
                sendMessage();
            }
        }));
        this.send = Button(id("chatSend"), disabled, onClick(sendMessage));
        Object.seal(this);
    }
    /**
     *
    ...
    set(user) {
        ...
        const elems = [
            ...
            Div(gridPos(1, row, 2, 1), zIndex(1), unhoveredColor, onMouseOver(function () {
    ...
    warn(...rest) {
        const elem = Div(gridPos(1, this.rows.size + 1, 2, 1), backgroundColor("yellow"), ...rest.map(i => i.toString()));
        this.usersList.append(elem);
        setTimeout(() => {
            this.usersList.removeChild(elem);
        }, 5000);
