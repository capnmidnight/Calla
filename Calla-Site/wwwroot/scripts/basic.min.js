 ...
 */function Y(){const t=`[Omnitone] ${Array.prototype.slice.call(arguments).join(" ")} (${performance.now().toFixed(2)}ms)`;window.console.log(t)}function q(){const t=`[Omnitone] ${Array.prototype.slice.call(arguments).join(" ")} (${performance.now().toFixed(2)}ms)`;throw window.console.error(t),new Error(t)}function X(t){return t instanceof AudioContext||t instanceof OfflineAudioContext}
/**
 * @license
 ...
 * limitations under the License.
 */
const Z={BASE64:"base64",URL:"url"};class W{constructor(t,e,i){X(t)||q("BufferList: Invalid BaseAudioContext."),this._context=t,this._options={dataType:Z.BASE64,verbose:!1},i&&(i.dataType&&function(t,e){for(let i in t)if(e===t[i])return!0;return!1}(Z,i.dataType)&&(this._options.dataType=i.dataType),i.verbose&&(this._options.verbose=Boolean(i.verbose))),this._bufferData=this._options.dataType===Z.BASE64?e:e.slice(0)}async load(){try{const t=this._bufferData.map((t,e)=>this._launchAsyncLoadTask(t,e)),e=await Promise.all(t);return Y("BufferList: "+(this._options.dataType===Z.BASE64?this._bufferData.length+" AudioBuffers from Base64-encoded HRIRs":this._bufferData.length+" files via XHR")+" loaded successfully."),e}catch(t){q('BufferList: error while loading "'+bData+'". ('+t.message+")")}}async _launchAsyncLoadTask(t,e){const i=await this._fetch(t),n=await this._context.decodeAudioData(i);return Y("BufferList: "+(this._options.dataType===Z.BASE64?"ArrayBuffer("+e+") from Base64-encoded HRIR":'"'+t+'"')+" successfully loaded."),n}async _fetch(t){if(this._options.dataType===Z.BASE64)return function(t){const e=window.atob(t),i=new Uint8Array(e.length);return i.forEach((t,n)=>i[n]=e.charCodeAt(n)),i.buffer}(t);{const e=await fetch(t);return await e.arrayBuffer()}}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const $={DEFAULT:[0,1,2,3],SAFARI:[2,0,1,3],FUMA:[0,3,1,2]};class tt{constructor(t,e){this._context=t,this._splitter=this._context.createChannelSplitter(4),this._merger=this._context.createChannelMerger(4),this.input=this._splitter,this.output=this._merger,this.setChannelMap(e||$.DEFAULT)}setChannelMap(t){Array.isArray(t)&&(this._channelMap=t,this._splitter.disconnect(),this._splitter.connect(this._merger,0,this._channelMap[0]),this._splitter.connect(this._merger,1,this._channelMap[1]),this._splitter.connect(this._merger,2,this._channelMap[2]),this._splitter.connect(this._merger,3,this._channelMap[3]))}dipose(){this._splitter.disconnect(this._merger,0,this._channelMap[0]),this._splitter.disconnect(this._merger,1,this._channelMap[1]),this._splitter.disconnect(this._merger,2,this._channelMap[2]),this._splitter.disconnect(this._merger,3,this._channelMap[3])}}tt.ChannelMap=$;var et=Object.freeze({AMBISONIC:"ambisonic",BYPASS:"bypass",OFF:"off"});
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class it{constructor(t,e){X(t)||q("FOARenderer: Invalid BaseAudioContext."),this._context=t,this._config={channelMap:tt.ChannelMap.DEFAULT,renderingMode:et.AMBISONIC},e&&(e.channelMap&&(Array.isArray(e.channelMap)&&4===e.channelMap.length?this._config.channelMap=e.channelMap:q("FOARenderer: Invalid channel map. (got "+e.channelMap+")")),e.hrirPathList&&(Array.isArray(e.hrirPathList)&&2===e.hrirPathList.length?this._config.pathList=e.hrirPathList:q("FOARenderer: Invalid HRIR URLs. It must be an array with 2 URLs to HRIR files. (got "+e.hrirPathList+")")),e.renderingMode&&(Object.values(et).includes(e.renderingMode)?this._config.renderingMode=e.renderingMode:Y("FOARenderer: Invalid rendering mode order. (got"+e.renderingMode+') Fallbacks to the mode "ambisonic".'))),this._buildAudioGraph(),this._tempMatrix4=new Float32Array(16)}_buildAudioGraph(){this.input=this._context.createGain(),this.output=this._context.createGain(),this._bypass=this._context.createGain(),this._foaRouter=new tt(this._context,this._config.channelMap),this._foaRotator=new K(this._context),this._foaConvolver=new J(this._context),this.input.connect(this._foaRouter.input),this.input.connect(this._bypass),this._foaRouter.output.connect(this._foaRotator.input),this._foaRotator.output.connect(this._foaConvolver.input),this._foaConvolver.output.connect(this.output),this.input.channelCount=4,this.input.channelCountMode="explicit",this.input.channelInterpretation="discrete"}dipose(){mode===et.BYPASS&&this._bypass.connect(this.output),this.input.disconnect(this._foaRouter.input),this.input.disconnect(this._bypass),this._foaRouter.output.disconnect(this._foaRotator.input),this._foaRotator.output.disconnect(this._foaConvolver.input),this._foaConvolver.output.disconnect(this.output),this._foaConvolver.dispose(),this._foaRotator.dispose(),this._foaRouter.dipose()}async initialize(){Y("FOARenderer: Initializing... (mode: "+this._config.renderingMode+")");const t=this._config.pathList?new W(this._context,this._config.pathList,{dataType:"url"}):new W(this._context,V);try{const e=await t.load();this._foaConvolver.setHRIRBufferList(e),this.setRenderingMode(this._config.renderingMode),Y("FOARenderer: HRIRs loaded successfully. Ready.")}catch(t){q("FOARenderer: HRIR loading/decoding failed. Reason: "+t.message)}}setChannelMap(t){t.toString()!==this._config.channelMap.toString()&&(Y("Remapping channels (["+this._config.channelMap.toString()+"] -> ["+t.toString()+"])."),this._config.channelMap=t.slice(),this._foaRouter.setChannelMap(this._config.channelMap))}setRotationMatrix3(t){this._foaRotator.setRotationMatrix3(t)}setRotationMatrix4(t){this._foaRotator.setRotationMatrix4(t)}getRenderingMode(){return this._config.renderingMode}setRenderingMode(t){if(t!==this._config.renderingMode){switch(t){case et.AMBISONIC:this._foaConvolver.enable(),this._bypass.disconnect();break;case et.BYPASS:this._foaConvolver.disable(),this._bypass.connect(this.output);break;case et.OFF:this._foaConvolver.disable(),this._bypass.disconnect();break;default:return void Y('FOARenderer: Rendering mode "'+t+'" is not supported.')}this._config.renderingMode=t,Y("FOARenderer: Rendering mode changed. ("+t+")")}}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * limitations under the License.
 */
class Rt{constructor(t,e){null==e&&(e={}),null==e.ambisonicOrder&&(e.ambisonicOrder=1),null==e.azimuth&&(e.azimuth=0),null==e.elevation&&(e.elevation=0),null==e.sourceWidth&&(e.sourceWidth=0),this._context=t,this.input=t.createGain(),this._channelGain=[],this._merger=void 0,this.output=t.createGain(),this.setAmbisonicOrder(e.ambisonicOrder),this._azimuth=e.azimuth,this._elevation=e.elevation,this.setSourceWidth(e.sourceWidth)}setAmbisonicOrder(t){this._ambisonicOrder=Rt.validateAmbisonicOrder(t),this.input.disconnect();for(let t=0;t<this._channelGain.length;t++)this._channelGain[t].disconnect();null!=this._merger&&this._merger.disconnect(),delete this._channelGain,delete this._merger;let e=(this._ambisonicOrder+1)*(this._ambisonicOrder+1);this._merger=this._context.createChannelMerger(e),this._channelGain=new Array(e);for(let t=0;t<e;t++)this._channelGain[t]=this._context.createGain(),this.input.connect(this._channelGain[t]),this._channelGain[t].connect(this._merger,0,t);this._merger.connect(this.output)}dispose(){this._merger.disconnect(this.output);let t=(this._ambisonicOrder+1)*(this._ambisonicOrder+1);for(let e=0;e<t;++e)this._channelGain[e].disconnect(this._merger,0,e),this.input.disconnect(this._channelGain[e])}setDirection(t,e){(null==t||isNaN(t))&&(t=0),(null==e||isNaN(e))&&(e=0),this._azimuth=t,this._elevation=e,(t=Math.round(t%360))<0&&(t+=360),e=Math.round(Math.min(90,Math.max(-90,e)))+90,this._channelGain[0].gain.value=Dt[this._spreadIndex][0];for(let i=1;i<=this._ambisonicOrder;i++){let n=Dt[this._spreadIndex][i];for(let s=-i;s<=i;s++){let r=i*i+i+s,o=i*(i+1)/2+Math.abs(s)-1,a=_t[1][e][o];if(0!=s){let e=xt+s-1;s<0&&(e=xt+s),a*=_t[0][t][e]}this._channelGain[r].gain.value=a*n}}}setSourceWidth(t){this._spreadIndex=Math.min(359,Math.max(0,Math.round(t))),this.setDirection(this._azimuth,this._elevation)}}Rt.validateAmbisonicOrder=function(t){return isNaN(t)||null==t?(St("Error: Invalid ambisonic order",options.ambisonicOrder,"\nUsing ambisonicOrder=1 instead."),t=1):t<1?(St("Error: Unable to render ambisonic order",options.ambisonicOrder,"(Min order is 1)","\nUsing min order instead."),t=1):t>xt&&(St("Error: Unable to render ambisonic order",options.ambisonicOrder,"(Max order is",xt,")\nUsing max order instead."),options.ambisonicOrder=xt),t};
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Ht{constructor(t,e){null==e&&(e={}),null==e.alpha&&(e.alpha=0),null==e.sharpness&&(e.sharpness=1),this._context=t,this._lowpass=t.createBiquadFilter(),this._lowpass.type="lowpass",this._lowpass.Q.value=0,this._lowpass.frequency.value=.5*t.sampleRate,this._cosTheta=0,this.setPattern(e.alpha,e.sharpness),this.input=this._lowpass,this.output=this._lowpass}computeAngle(t,e){let i=Nt(t),n=Nt(e),s=1;if(this._alpha>1e-8){let t=i[0]*n[0]+i[1]*n[1]+i[2]*n[2];s=1-this._alpha+this._alpha*t,s=Math.pow(Math.abs(s),this._sharpness)}this._lowpass.frequency.value=.5*this._context.sampleRate*s}setPattern(t,e){this._alpha=Math.min(1,Math.max(0,t)),this._sharpness=Math.max(1,e),this.computeAngle([this._cosTheta*this._cosTheta,0,0],[1,0,0])}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Ft{constructor(t,e){null==e&&(e={}),null==e.minDistance&&(e.minDistance=1),null==e.maxDistance&&(e.maxDistance=1e3),null==e.rolloff&&(e.rolloff="logarithmic"),this.minDistance=e.minDistance,this.maxDistance=e.maxDistance,this.setRolloff(e.rolloff),this._gainNode=t.createGain(),this.setDistance(e.maxDistance),this.input=this._gainNode,this.output=this._gainNode}setDistance(t){let e=1;if("logarithmic"==this._rolloff){if(t>this.maxDistance)e=0;else if(t>this.minDistance){let i=this.maxDistance-this.minDistance;if(i>1e-8){let n=1/(i+1);e=(1/(t-this.minDistance+1)-n)/(1-n)}}}else if("linear"==this._rolloff)if(t>this.maxDistance)e=0;else if(t>this.minDistance){let i=this.maxDistance-this.minDistance;i>1e-8&&(e=(this.maxDistance-t)/i)}this._gainNode.gain.value=e}setRolloff(t){let e=~Et.indexOf(t);null!=t&&e?t=t.toString().toLowerCase():(e||St('Invalid rolloff model ("'+t+'"). Using default: "logarithmic".'),t="logarithmic"),this._rolloff=t}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function qt(t){let e={};for(let t in zt)zt.hasOwnProperty(t)&&(e[t]=Ot[zt[t]]);null==t&&(t={},Object.assign(t,zt));for(let i in zt)zt.hasOwnProperty(i)&&t.hasOwnProperty(i)?t[i]in Ot?e[i]=Ot[t[i]]:St('Material "'+t[i]+'" on wall "'+i+'" not found. Using "'+zt[i]+'".'):St('Wall "'+i+'" is not defined. Default used.');return e}function Xt(t){null==t&&(t={});for(let e in Bt)t.hasOwnProperty(e)||(t[e]=Bt[e]);return t}function Zt(t,e,i){let n=new Float32Array(Tt);t=Xt(t),e=function(t){null==t&&(t={});for(let e in zt)t.hasOwnProperty(e)||(t[e]=Ot[zt[e]]);return t}(e),null==i&&(i=343);let s=55.2620422318571/i,r=t.width*t.height*t.depth;if(r<1e-4)return n;let o=t.width*t.height,a=t.width*t.depth,A=t.depth*t.height,c=2*(o+a+A);for(let t=0;t<Tt;t++){let i=((e.left[t]+e.right[t])*o+(e.down[t]+e.up[t])*a+(e.front[t]+e.back[t])*A)/c;n[t]=1.38*s*r/(-c*Math.log(1-i)+4*Lt[t]*r)}return n}function Wt(t){let e=[];for(let i in Mt)if(Mt.hasOwnProperty(i)){e[i]=0;for(let n=0;n<3;n++){let s=n+4;e[i]+=t[i][s]}e[i]/=3,e[i]=Math.sqrt(1-e[i])}return e}class Jt{constructor(t,e){null==e&&(e={}),null==e.listenerPosition&&(e.listenerPosition=Pt.slice()),null==e.dimensions&&(e.dimensions={},Object.assign(e.dimensions,Bt)),null==e.materials&&(e.materials={},Object.assign(e.materials,zt)),null==e.speedOfSound&&(e.speedOfSound=343),e.dimensions=Xt(e.dimensions);let i=qt(e.materials),n=Wt(i),s=Zt(e.dimensions,i,e.speedOfSound);this.early=new Yt(t,{dimensions:e.dimensions,coefficients:n,speedOfSound:e.speedOfSound,listenerPosition:e.listenerPosition}),this.late=new Gt(t,{durations:s}),this.speedOfSound=e.speedOfSound,this.output=t.createGain(),this.early.output.connect(this.output),this._merger=t.createChannelMerger(4),this.late.output.connect(this._merger,0,0),this._merger.connect(this.output)}dispose(){this.early.output.disconnect(this.output),this.late.output.disconnect(this._merger,0,0),this._merger.disconnect(this.output)}setProperties(t,e){let i=qt(e),n=Zt(t,i,this.speedOfSound);this.late.setDurations(n),this.early.speedOfSound=this.speedOfSound;let s=Wt(i);this.early.setRoomProperties(t,s)}setListenerPosition(t,e,i){this.early.speedOfSound=this.speedOfSound,this.early.setListenerPosition(t,e,i);let n=this.getDistanceOutsideRoom(t,e,i),s=1;n>1e-8&&(s=1-n/1,s=Math.max(0,Math.min(1,s))),this.output.gain.value=s}getDistanceOutsideRoom(t,e,i){let n=Math.max(0,-this.early._halfDimensions.width-t,t-this.early._halfDimensions.width),s=Math.max(0,-this.early._halfDimensions.height-e,e-this.early._halfDimensions.height),r=Math.max(0,-this.early._halfDimensions.depth-i,i-this.early._halfDimensions.depth);return Math.sqrt(n*n+s*s+r*r)}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
