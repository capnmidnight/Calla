 ...
 */function U(){const t=`[Omnitone] ${Array.prototype.slice.call(arguments).join(" ")} (${performance.now().toFixed(2)}ms)`;window.console.log(t)}function G(){const t=`[Omnitone] ${Array.prototype.slice.call(arguments).join(" ")} (${performance.now().toFixed(2)}ms)`;throw window.console.error(t),new Error(t)}let q,Y,X,W,Z,J,V,K,$,tt,et,it,nt,st,rt,ot,at,At,ct,ht,ut,lt,dt,ft,pt,gt,vt,mt,wt;function yt(t){return t instanceof AudioContext||t instanceof OfflineAudioContext}
/**
 * @license
 ...
 * limitations under the License.
 */
const _t={BASE64:"base64",URL:"url"};class xt{constructor(t,e,i){yt(t)||G("BufferList: Invalid BaseAudioContext."),this._context=t,this._options={dataType:_t.BASE64,verbose:!1},i&&(i.dataType&&function(t,e){for(let i in t)if(e===t[i])return!0;return!1}(_t,i.dataType)&&(this._options.dataType=i.dataType),i.verbose&&(this._options.verbose=Boolean(i.verbose))),this._bufferData=this._options.dataType===_t.BASE64?e:e.slice(0)}async load(){try{const t=this._bufferData.map((t,e)=>this._launchAsyncLoadTask(t,e)),e=await Promise.all(t);return U("BufferList: "+(this._options.dataType===_t.BASE64?this._bufferData.length+" AudioBuffers from Base64-encoded HRIRs":this._bufferData.length+" files via XHR")+" loaded successfully."),e}catch(t){G('BufferList: error while loading "'+bData+'". ('+t.message+")")}}async _launchAsyncLoadTask(t,e){const i=await this._fetch(t),n=await this._context.decodeAudioData(i);return U("BufferList: "+(this._options.dataType===_t.BASE64?"ArrayBuffer("+e+") from Base64-encoded HRIR":'"'+t+'"')+" successfully loaded."),n}async _fetch(t){if(this._options.dataType===_t.BASE64)return function(t){const e=window.atob(t),i=new Uint8Array(e.length);return i.forEach((t,n)=>i[n]=e.charCodeAt(n)),i.buffer}(t);{const e=await fetch(t);return await e.arrayBuffer()}}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const bt={DEFAULT:[0,1,2,3],SAFARI:[2,0,1,3],FUMA:[0,3,1,2]};class Et{constructor(t,e){this._context=t,this._splitter=this._context.createChannelSplitter(4),this._merger=this._context.createChannelMerger(4),this.input=this._splitter,this.output=this._merger,this.setChannelMap(e||bt.DEFAULT)}setChannelMap(t){Array.isArray(t)&&(this._channelMap=t,this._splitter.disconnect(),this._splitter.connect(this._merger,0,this._channelMap[0]),this._splitter.connect(this._merger,1,this._channelMap[1]),this._splitter.connect(this._merger,2,this._channelMap[2]),this._splitter.connect(this._merger,3,this._channelMap[3]))}dipose(){this._splitter.disconnect(this._merger,0,this._channelMap[0]),this._splitter.disconnect(this._merger,1,this._channelMap[1]),this._splitter.disconnect(this._merger,2,this._channelMap[2]),this._splitter.disconnect(this._merger,3,this._channelMap[3])}}Et.ChannelMap=bt;var Mt=Object.freeze({AMBISONIC:"ambisonic",BYPASS:"bypass",OFF:"off"});
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Bt{constructor(t,e){yt(t)||G("FOARenderer: Invalid BaseAudioContext."),this._context=t,this._config={channelMap:Et.ChannelMap.DEFAULT,renderingMode:Mt.AMBISONIC},e&&(e.channelMap&&(Array.isArray(e.channelMap)&&4===e.channelMap.length?this._config.channelMap=e.channelMap:G("FOARenderer: Invalid channel map. (got "+e.channelMap+")")),e.hrirPathList&&(Array.isArray(e.hrirPathList)&&2===e.hrirPathList.length?this._config.pathList=e.hrirPathList:G("FOARenderer: Invalid HRIR URLs. It must be an array with 2 URLs to HRIR files. (got "+e.hrirPathList+")")),e.renderingMode&&(Object.values(Mt).includes(e.renderingMode)?this._config.renderingMode=e.renderingMode:U("FOARenderer: Invalid rendering mode order. (got"+e.renderingMode+') Fallbacks to the mode "ambisonic".'))),this._buildAudioGraph(),this._tempMatrix4=new Float32Array(16)}_buildAudioGraph(){this.input=this._context.createGain(),this.output=this._context.createGain(),this._bypass=this._context.createGain(),this._foaRouter=new Et(this._context,this._config.channelMap),this._foaRotator=new Ct(this._context),this._foaConvolver=new Dt(this._context),this.input.connect(this._foaRouter.input),this.input.connect(this._bypass),this._foaRouter.output.connect(this._foaRotator.input),this._foaRotator.output.connect(this._foaConvolver.input),this._foaConvolver.output.connect(this.output),this.input.channelCount=4,this.input.channelCountMode="explicit",this.input.channelInterpretation="discrete"}dipose(){mode===Mt.BYPASS&&this._bypass.connect(this.output),this.input.disconnect(this._foaRouter.input),this.input.disconnect(this._bypass),this._foaRouter.output.disconnect(this._foaRotator.input),this._foaRotator.output.disconnect(this._foaConvolver.input),this._foaConvolver.output.disconnect(this.output),this._foaConvolver.dispose(),this._foaRotator.dispose(),this._foaRouter.dipose()}async initialize(){U("FOARenderer: Initializing... (mode: "+this._config.renderingMode+")");const t=this._config.pathList?new xt(this._context,this._config.pathList,{dataType:"url"}):new xt(this._context,Pt);try{const e=await t.load();this._foaConvolver.setHRIRBufferList(e),this.setRenderingMode(this._config.renderingMode),U("FOARenderer: HRIRs loaded successfully. Ready.")}catch(t){G("FOARenderer: HRIR loading/decoding failed. Reason: "+t.message)}}setChannelMap(t){t.toString()!==this._config.channelMap.toString()&&(U("Remapping channels (["+this._config.channelMap.toString()+"] -> ["+t.toString()+"])."),this._config.channelMap=t.slice(),this._foaRouter.setChannelMap(this._config.channelMap))}setRotationMatrix3(t){this._foaRotator.setRotationMatrix3(t)}setRotationMatrix4(t){this._foaRotator.setRotationMatrix4(t)}setRotationMatrixFromCamera(t){var e,i;e=this._tempMatrix4,i=t.elements,q=i[0],Y=i[1],X=i[2],W=i[3],Z=i[4],J=i[5],V=i[6],K=i[7],$=i[8],tt=i[9],et=i[10],it=i[11],nt=i[12],st=i[13],rt=i[14],ot=i[15],at=q*J-Y*Z,At=q*V-X*Z,ct=q*K-W*Z,ht=Y*V-X*J,ut=Y*K-W*J,lt=X*K-W*V,dt=$*st-tt*nt,ft=$*rt-et*nt,pt=$*ot-it*nt,gt=tt*rt-et*st,vt=tt*ot-it*st,mt=et*ot-it*rt,wt=at*mt-At*vt+ct*gt+ht*pt-ut*ft+lt*dt,wt&&(wt=1/wt,e[0]=(J*mt-V*vt+K*gt)*wt,e[1]=(X*vt-Y*mt-W*gt)*wt,e[2]=(st*lt-rt*ut+ot*ht)*wt,e[3]=(et*ut-tt*lt-it*ht)*wt,e[4]=(V*pt-Z*mt-K*ft)*wt,e[5]=(q*mt-X*pt+W*ft)*wt,e[6]=(rt*ct-nt*lt-ot*At)*wt,e[7]=($*lt-et*ct+it*At)*wt,e[8]=(Z*vt-J*pt+K*dt)*wt,e[9]=(Y*pt-q*vt-W*dt)*wt,e[10]=(nt*ut-st*ct+ot*at)*wt,e[11]=(tt*ct-$*ut-it*at)*wt,e[12]=(J*ft-Z*gt-V*dt)*wt,e[13]=(q*gt-Y*ft+X*dt)*wt,e[14]=(st*At-nt*ht-rt*at)*wt,e[15]=($*ht-tt*At+et*at)*wt),this._foaRotator.setRotationMatrix4(this._tempMatrix4)}getRenderingMode(){return this._config.renderingMode}setRenderingMode(t){if(t!==this._config.renderingMode){switch(t){case Mt.AMBISONIC:this._foaConvolver.enable(),this._bypass.disconnect();break;case Mt.BYPASS:this._foaConvolver.disable(),this._bypass.connect(this.output);break;case Mt.OFF:this._foaConvolver.disable(),this._bypass.disconnect();break;default:return void U('FOARenderer: Rendering mode "'+t+'" is not supported.')}this._config.renderingMode=t,U("FOARenderer: Rendering mode changed. ("+t+")")}}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * limitations under the License.
 */
class ue{constructor(t,e){null==e&&(e={}),null==e.ambisonicOrder&&(e.ambisonicOrder=1),null==e.azimuth&&(e.azimuth=0),null==e.elevation&&(e.elevation=0),null==e.sourceWidth&&(e.sourceWidth=0),this._context=t,this.input=t.createGain(),this._channelGain=[],this._merger=void 0,this.output=t.createGain(),this.setAmbisonicOrder(e.ambisonicOrder),this._azimuth=e.azimuth,this._elevation=e.elevation,this.setSourceWidth(e.sourceWidth)}setAmbisonicOrder(t){this._ambisonicOrder=ue.validateAmbisonicOrder(t),this.input.disconnect();for(let t=0;t<this._channelGain.length;t++)this._channelGain[t].disconnect();null!=this._merger&&this._merger.disconnect(),delete this._channelGain,delete this._merger;let e=(this._ambisonicOrder+1)*(this._ambisonicOrder+1);this._merger=this._context.createChannelMerger(e),this._channelGain=new Array(e);for(let t=0;t<e;t++)this._channelGain[t]=this._context.createGain(),this.input.connect(this._channelGain[t]),this._channelGain[t].connect(this._merger,0,t);this._merger.connect(this.output)}dispose(){this._merger.disconnect(this.output);let t=(this._ambisonicOrder+1)*(this._ambisonicOrder+1);for(let e=0;e<t;++e)this._channelGain[e].disconnect(this._merger,0,e),this.input.disconnect(this._channelGain[e])}setDirection(t,e){(null==t||isNaN(t))&&(t=0),(null==e||isNaN(e))&&(e=0),this._azimuth=t,this._elevation=e,(t=Math.round(t%360))<0&&(t+=360),e=Math.round(Math.min(90,Math.max(-90,e)))+90,this._channelGain[0].gain.value=Zt[this._spreadIndex][0];for(let i=1;i<=this._ambisonicOrder;i++){let n=Zt[this._spreadIndex][i];for(let s=-i;s<=i;s++){let r=i*i+i+s,o=i*(i+1)/2+Math.abs(s)-1,a=Xt[1][e][o];if(0!=s){let e=Wt+s-1;s<0&&(e=Wt+s),a*=Xt[0][t][e]}this._channelGain[r].gain.value=a*n}}}setSourceWidth(t){this._spreadIndex=Math.min(359,Math.max(0,Math.round(t))),this.setDirection(this._azimuth,this._elevation)}}ue.validateAmbisonicOrder=function(t){return isNaN(t)||null==t?(Ae("Error: Invalid ambisonic order",options.ambisonicOrder,"\nUsing ambisonicOrder=1 instead."),t=1):t<1?(Ae("Error: Unable to render ambisonic order",options.ambisonicOrder,"(Min order is 1)","\nUsing min order instead."),t=1):t>Wt&&(Ae("Error: Unable to render ambisonic order",options.ambisonicOrder,"(Max order is",Wt,")\nUsing max order instead."),options.ambisonicOrder=Wt),t};
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class de{constructor(t,e){null==e&&(e={}),null==e.alpha&&(e.alpha=0),null==e.sharpness&&(e.sharpness=1),this._context=t,this._lowpass=t.createBiquadFilter(),this._lowpass.type="lowpass",this._lowpass.Q.value=0,this._lowpass.frequency.value=.5*t.sampleRate,this._cosTheta=0,this.setPattern(e.alpha,e.sharpness),this.input=this._lowpass,this.output=this._lowpass}computeAngle(t,e){let i=ce(t),n=ce(e),s=1;if(this._alpha>1e-8){let t=i[0]*n[0]+i[1]*n[1]+i[2]*n[2];s=1-this._alpha+this._alpha*t,s=Math.pow(Math.abs(s),this._sharpness)}this._lowpass.frequency.value=.5*this._context.sampleRate*s}setPattern(t,e){this._alpha=Math.min(1,Math.max(0,t)),this._sharpness=Math.max(1,e),this.computeAngle([this._cosTheta*this._cosTheta,0,0],[1,0,0])}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class fe{constructor(t,e){null==e&&(e={}),null==e.minDistance&&(e.minDistance=1),null==e.maxDistance&&(e.maxDistance=1e3),null==e.rolloff&&(e.rolloff="logarithmic"),this.minDistance=e.minDistance,this.maxDistance=e.maxDistance,this.setRolloff(e.rolloff),this._gainNode=t.createGain(),this.setDistance(e.maxDistance),this.input=this._gainNode,this.output=this._gainNode}setDistance(t){let e=1;if("logarithmic"==this._rolloff){if(t>this.maxDistance)e=0;else if(t>this.minDistance){let i=this.maxDistance-this.minDistance;if(i>1e-8){let n=1/(i+1);e=(1/(t-this.minDistance+1)-n)/(1-n)}}}else if("linear"==this._rolloff)if(t>this.maxDistance)e=0;else if(t>this.minDistance){let i=this.maxDistance-this.minDistance;i>1e-8&&(e=(this.maxDistance-t)/i)}this._gainNode.gain.value=e}setRolloff(t){let e=~$t.indexOf(t);null!=t&&e?t=t.toString().toLowerCase():(e||Ae('Invalid rolloff model ("'+t+'"). Using default: "logarithmic".'),t="logarithmic"),this._rolloff=t}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function me(t){let e={};for(let t in oe)oe.hasOwnProperty(t)&&(e[t]=re[oe[t]]);null==t&&(t={},Object.assign(t,oe));for(let i in oe)oe.hasOwnProperty(i)&&t.hasOwnProperty(i)?t[i]in re?e[i]=re[t[i]]:Ae('Material "'+t[i]+'" on wall "'+i+'" not found. Using "'+oe[i]+'".'):Ae('Wall "'+i+'" is not defined. Default used.');return e}function we(t){null==t&&(t={});for(let e in ee)t.hasOwnProperty(e)||(t[e]=ee[e]);return t}function ye(t,e,i){let n=new Float32Array(ne);t=we(t),e=function(t){null==t&&(t={});for(let e in oe)t.hasOwnProperty(e)||(t[e]=re[oe[e]]);return t}(e),null==i&&(i=343);let s=55.2620422318571/i,r=t.width*t.height*t.depth;if(r<1e-4)return n;let o=t.width*t.height,a=t.width*t.depth,A=t.depth*t.height,c=2*(o+a+A);for(let t=0;t<ne;t++){let i=((e.left[t]+e.right[t])*o+(e.down[t]+e.up[t])*a+(e.front[t]+e.back[t])*A)/c;n[t]=1.38*s*r/(-c*Math.log(1-i)+4*ae[t]*r)}return n}function _e(t){let e=[];for(let i in te)if(te.hasOwnProperty(i)){e[i]=0;for(let n=0;n<3;n++){let s=n+4;e[i]+=t[i][s]}e[i]/=3,e[i]=Math.sqrt(1-e[i])}return e}class xe{constructor(t,e){null==e&&(e={}),null==e.listenerPosition&&(e.listenerPosition=Jt.slice()),null==e.dimensions&&(e.dimensions={},Object.assign(e.dimensions,ee)),null==e.materials&&(e.materials={},Object.assign(e.materials,oe)),null==e.speedOfSound&&(e.speedOfSound=343),e.dimensions=we(e.dimensions);let i=me(e.materials),n=_e(i),s=ye(e.dimensions,i,e.speedOfSound);this.early=new ve(t,{dimensions:e.dimensions,coefficients:n,speedOfSound:e.speedOfSound,listenerPosition:e.listenerPosition}),this.late=new ge(t,{durations:s}),this.speedOfSound=e.speedOfSound,this.output=t.createGain(),this.early.output.connect(this.output),this._merger=t.createChannelMerger(4),this.late.output.connect(this._merger,0,0),this._merger.connect(this.output)}dispose(){this.early.output.disconnect(this.output),this.late.output.disconnect(this._merger,0,0),this._merger.disconnect(this.output)}setProperties(t,e){let i=me(e),n=ye(t,i,this.speedOfSound);this.late.setDurations(n),this.early.speedOfSound=this.speedOfSound;let s=_e(i);this.early.setRoomProperties(t,s)}setListenerPosition(t,e,i){this.early.speedOfSound=this.speedOfSound,this.early.setListenerPosition(t,e,i);let n=this.getDistanceOutsideRoom(t,e,i),s=1;n>1e-8&&(s=1-n/1,s=Math.max(0,Math.min(1,s))),this.output.gain.value=s}getDistanceOutsideRoom(t,e,i){let n=Math.max(0,-this.early._halfDimensions.width-t,t-this.early._halfDimensions.width),s=Math.max(0,-this.early._halfDimensions.height-e,e-this.early._halfDimensions.height),r=Math.max(0,-this.early._halfDimensions.depth-i,i-this.early._halfDimensions.depth);return Math.sqrt(n*n+s*s+r*r)}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
