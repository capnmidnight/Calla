 ...
 */function L(){const t=`[Omnitone] ${Array.prototype.slice.call(arguments).join(" ")} (${performance.now().toFixed(2)}ms)`;window.console.log(t)}function N(){const t=`[Omnitone] ${Array.prototype.slice.call(arguments).join(" ")} (${performance.now().toFixed(2)}ms)`;throw window.console.error(t),new Error(t)}let k,R,j,F,H,U,G,Y,q,X,Z,W,J,V,K,$,tt,et,it,nt,st,rt,ot,at,At,ct,ht,ut,lt;function dt(t){return t instanceof AudioContext||t instanceof OfflineAudioContext}
/**
 * @license
 ...
 * limitations under the License.
 */
const ft={BASE64:"base64",URL:"url"};class pt{constructor(t,e,i){dt(t)||N("BufferList: Invalid BaseAudioContext."),this._context=t,this._options={dataType:ft.BASE64,verbose:!1},i&&(i.dataType&&function(t,e){for(let i in t)if(e===t[i])return!0;return!1}(ft,i.dataType)&&(this._options.dataType=i.dataType),i.verbose&&(this._options.verbose=Boolean(i.verbose))),this._bufferData=this._options.dataType===ft.BASE64?e:e.slice(0)}async load(){try{const t=this._bufferData.map((t,e)=>this._launchAsyncLoadTask(t,e)),e=await Promise.all(t);return L("BufferList: "+(this._options.dataType===ft.BASE64?this._bufferData.length+" AudioBuffers from Base64-encoded HRIRs":this._bufferData.length+" files via XHR")+" loaded successfully."),e}catch(t){N('BufferList: error while loading "'+bData+'". ('+t.message+")")}}async _launchAsyncLoadTask(t,e){const i=await this._fetch(t),n=await this._context.decodeAudioData(i);return L("BufferList: "+(this._options.dataType===ft.BASE64?"ArrayBuffer("+e+") from Base64-encoded HRIR":'"'+t+'"')+" successfully loaded."),n}async _fetch(t){if(this._options.dataType===ft.BASE64)return function(t){const e=window.atob(t),i=new Uint8Array(e.length);return i.forEach((t,n)=>i[n]=e.charCodeAt(n)),i.buffer}(t);{const e=await fetch(t);return await e.arrayBuffer()}}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const yt={DEFAULT:[0,1,2,3],SAFARI:[2,0,1,3],FUMA:[0,3,1,2]};class wt{constructor(t,e){this._context=t,this._splitter=this._context.createChannelSplitter(4),this._merger=this._context.createChannelMerger(4),this.input=this._splitter,this.output=this._merger,this.setChannelMap(e||yt.DEFAULT)}setChannelMap(t){Array.isArray(t)&&(this._channelMap=t,this._splitter.disconnect(),this._splitter.connect(this._merger,0,this._channelMap[0]),this._splitter.connect(this._merger,1,this._channelMap[1]),this._splitter.connect(this._merger,2,this._channelMap[2]),this._splitter.connect(this._merger,3,this._channelMap[3]))}dipose(){this._splitter.disconnect(this._merger,0,this._channelMap[0]),this._splitter.disconnect(this._merger,1,this._channelMap[1]),this._splitter.disconnect(this._merger,2,this._channelMap[2]),this._splitter.disconnect(this._merger,3,this._channelMap[3])}}wt.ChannelMap=yt;var _t=Object.freeze({AMBISONIC:"ambisonic",BYPASS:"bypass",OFF:"off"});
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class xt{constructor(t,e){dt(t)||N("FOARenderer: Invalid BaseAudioContext."),this._context=t,this._config={channelMap:wt.ChannelMap.DEFAULT,renderingMode:_t.AMBISONIC},e&&(e.channelMap&&(Array.isArray(e.channelMap)&&4===e.channelMap.length?this._config.channelMap=e.channelMap:N("FOARenderer: Invalid channel map. (got "+e.channelMap+")")),e.hrirPathList&&(Array.isArray(e.hrirPathList)&&2===e.hrirPathList.length?this._config.pathList=e.hrirPathList:N("FOARenderer: Invalid HRIR URLs. It must be an array with 2 URLs to HRIR files. (got "+e.hrirPathList+")")),e.renderingMode&&(Object.values(_t).includes(e.renderingMode)?this._config.renderingMode=e.renderingMode:L("FOARenderer: Invalid rendering mode order. (got"+e.renderingMode+') Fallbacks to the mode "ambisonic".'))),this._buildAudioGraph(),this._tempMatrix4=new Float32Array(16)}_buildAudioGraph(){this.input=this._context.createGain(),this.output=this._context.createGain(),this._bypass=this._context.createGain(),this._foaRouter=new wt(this._context,this._config.channelMap),this._foaRotator=new mt(this._context),this._foaConvolver=new gt(this._context),this.input.connect(this._foaRouter.input),this.input.connect(this._bypass),this._foaRouter.output.connect(this._foaRotator.input),this._foaRotator.output.connect(this._foaConvolver.input),this._foaConvolver.output.connect(this.output),this.input.channelCount=4,this.input.channelCountMode="explicit",this.input.channelInterpretation="discrete"}dipose(){mode===_t.BYPASS&&this._bypass.connect(this.output),this.input.disconnect(this._foaRouter.input),this.input.disconnect(this._bypass),this._foaRouter.output.disconnect(this._foaRotator.input),this._foaRotator.output.disconnect(this._foaConvolver.input),this._foaConvolver.output.disconnect(this.output),this._foaConvolver.dispose(),this._foaRotator.dispose(),this._foaRouter.dipose()}async initialize(){L("FOARenderer: Initializing... (mode: "+this._config.renderingMode+")");const t=this._config.pathList?new pt(this._context,this._config.pathList,{dataType:"url"}):new pt(this._context,vt);try{const e=await t.load();this._foaConvolver.setHRIRBufferList(e),this.setRenderingMode(this._config.renderingMode),L("FOARenderer: HRIRs loaded successfully. Ready.")}catch(t){N("FOARenderer: HRIR loading/decoding failed. Reason: "+t.message)}}setChannelMap(t){t.toString()!==this._config.channelMap.toString()&&(L("Remapping channels (["+this._config.channelMap.toString()+"] -> ["+t.toString()+"])."),this._config.channelMap=t.slice(),this._foaRouter.setChannelMap(this._config.channelMap))}setRotationMatrix3(t){this._foaRotator.setRotationMatrix3(t)}setRotationMatrix4(t){this._foaRotator.setRotationMatrix4(t)}setRotationMatrixFromCamera(t){var e,i;e=this._tempMatrix4,i=t.elements,k=i[0],R=i[1],j=i[2],F=i[3],H=i[4],U=i[5],G=i[6],Y=i[7],q=i[8],X=i[9],Z=i[10],W=i[11],J=i[12],V=i[13],K=i[14],$=i[15],tt=k*U-R*H,et=k*G-j*H,it=k*Y-F*H,nt=R*G-j*U,st=R*Y-F*U,rt=j*Y-F*G,ot=q*V-X*J,at=q*K-Z*J,At=q*$-W*J,ct=X*K-Z*V,ht=X*$-W*V,ut=Z*$-W*K,lt=tt*ut-et*ht+it*ct+nt*At-st*at+rt*ot,lt&&(lt=1/lt,e[0]=(U*ut-G*ht+Y*ct)*lt,e[1]=(j*ht-R*ut-F*ct)*lt,e[2]=(V*rt-K*st+$*nt)*lt,e[3]=(Z*st-X*rt-W*nt)*lt,e[4]=(G*At-H*ut-Y*at)*lt,e[5]=(k*ut-j*At+F*at)*lt,e[6]=(K*it-J*rt-$*et)*lt,e[7]=(q*rt-Z*it+W*et)*lt,e[8]=(H*ht-U*At+Y*ot)*lt,e[9]=(R*At-k*ht-F*ot)*lt,e[10]=(J*st-V*it+$*tt)*lt,e[11]=(X*it-q*st-W*tt)*lt,e[12]=(U*at-H*ct-G*ot)*lt,e[13]=(k*ct-R*at+j*ot)*lt,e[14]=(V*et-J*nt-K*tt)*lt,e[15]=(q*nt-X*et+Z*tt)*lt),this._foaRotator.setRotationMatrix4(this._tempMatrix4)}getRenderingMode(){return this._config.renderingMode}setRenderingMode(t){if(t!==this._config.renderingMode){switch(t){case _t.AMBISONIC:this._foaConvolver.enable(),this._bypass.disconnect();break;case _t.BYPASS:this._foaConvolver.disable(),this._bypass.connect(this.output);break;case _t.OFF:this._foaConvolver.disable(),this._bypass.disconnect();break;default:return void L('FOARenderer: Rendering mode "'+t+'" is not supported.')}this._config.renderingMode=t,L("FOARenderer: Rendering mode changed. ("+t+")")}}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * limitations under the License.
 */
class se{constructor(t,e){null==e&&(e={}),null==e.ambisonicOrder&&(e.ambisonicOrder=1),null==e.azimuth&&(e.azimuth=0),null==e.elevation&&(e.elevation=0),null==e.sourceWidth&&(e.sourceWidth=0),this._context=t,this.input=t.createGain(),this._channelGain=[],this._merger=void 0,this.output=t.createGain(),this.setAmbisonicOrder(e.ambisonicOrder),this._azimuth=e.azimuth,this._elevation=e.elevation,this.setSourceWidth(e.sourceWidth)}setAmbisonicOrder(t){this._ambisonicOrder=se.validateAmbisonicOrder(t),this.input.disconnect();for(let t=0;t<this._channelGain.length;t++)this._channelGain[t].disconnect();null!=this._merger&&this._merger.disconnect(),delete this._channelGain,delete this._merger;let e=(this._ambisonicOrder+1)*(this._ambisonicOrder+1);this._merger=this._context.createChannelMerger(e),this._channelGain=new Array(e);for(let t=0;t<e;t++)this._channelGain[t]=this._context.createGain(),this.input.connect(this._channelGain[t]),this._channelGain[t].connect(this._merger,0,t);this._merger.connect(this.output)}dispose(){this._merger.disconnect(this.output);let t=(this._ambisonicOrder+1)*(this._ambisonicOrder+1);for(let e=0;e<t;++e)this._channelGain[e].disconnect(this._merger,0,e),this.input.disconnect(this._channelGain[e])}setDirection(t,e){(null==t||isNaN(t))&&(t=0),(null==e||isNaN(e))&&(e=0),this._azimuth=t,this._elevation=e,(t=Math.round(t%360))<0&&(t+=360),e=Math.round(Math.min(90,Math.max(-90,e)))+90,this._channelGain[0].gain.value=Ht[this._spreadIndex][0];for(let i=1;i<=this._ambisonicOrder;i++){let n=Ht[this._spreadIndex][i];for(let s=-i;s<=i;s++){let r=i*i+i+s,o=i*(i+1)/2+Math.abs(s)-1,a=jt[1][e][o];if(0!=s){let e=Ft+s-1;s<0&&(e=Ft+s),a*=jt[0][t][e]}this._channelGain[r].gain.value=a*n}}}setSourceWidth(t){this._spreadIndex=Math.min(359,Math.max(0,Math.round(t))),this.setDirection(this._azimuth,this._elevation)}}se.validateAmbisonicOrder=function(t){return isNaN(t)||null==t?(ee("Error: Invalid ambisonic order",options.ambisonicOrder,"\nUsing ambisonicOrder=1 instead."),t=1):t<1?(ee("Error: Unable to render ambisonic order",options.ambisonicOrder,"(Min order is 1)","\nUsing min order instead."),t=1):t>Ft&&(ee("Error: Unable to render ambisonic order",options.ambisonicOrder,"(Max order is",Ft,")\nUsing max order instead."),options.ambisonicOrder=Ft),t};
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class oe{constructor(t,e){null==e&&(e={}),null==e.alpha&&(e.alpha=0),null==e.sharpness&&(e.sharpness=1),this._context=t,this._lowpass=t.createBiquadFilter(),this._lowpass.type="lowpass",this._lowpass.Q.value=0,this._lowpass.frequency.value=.5*t.sampleRate,this._cosTheta=0,this.setPattern(e.alpha,e.sharpness),this.input=this._lowpass,this.output=this._lowpass}computeAngle(t,e){let i=ie(t),n=ie(e),s=1;if(this._alpha>1e-8){let t=i[0]*n[0]+i[1]*n[1]+i[2]*n[2];s=1-this._alpha+this._alpha*t,s=Math.pow(Math.abs(s),this._sharpness)}this._lowpass.frequency.value=.5*this._context.sampleRate*s}setPattern(t,e){this._alpha=Math.min(1,Math.max(0,t)),this._sharpness=Math.max(1,e),this.computeAngle([this._cosTheta*this._cosTheta,0,0],[1,0,0])}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class ae{constructor(t,e){null==e&&(e={}),null==e.minDistance&&(e.minDistance=1),null==e.maxDistance&&(e.maxDistance=1e3),null==e.rolloff&&(e.rolloff="logarithmic"),this.minDistance=e.minDistance,this.maxDistance=e.maxDistance,this.setRolloff(e.rolloff),this._gainNode=t.createGain(),this.setDistance(e.maxDistance),this.input=this._gainNode,this.output=this._gainNode}setDistance(t){let e=1;if("logarithmic"==this._rolloff){if(t>this.maxDistance)e=0;else if(t>this.minDistance){let i=this.maxDistance-this.minDistance;if(i>1e-8){let n=1/(i+1);e=(1/(t-this.minDistance+1)-n)/(1-n)}}}else if("linear"==this._rolloff)if(t>this.maxDistance)e=0;else if(t>this.minDistance){let i=this.maxDistance-this.minDistance;i>1e-8&&(e=(this.maxDistance-t)/i)}this._gainNode.gain.value=e}setRolloff(t){let e=~qt.indexOf(t);null!=t&&e?t=t.toString().toLowerCase():(e||ee('Invalid rolloff model ("'+t+'"). Using default: "logarithmic".'),t="logarithmic"),this._rolloff=t}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function ue(t){let e={};for(let t in $t)$t.hasOwnProperty(t)&&(e[t]=Kt[$t[t]]);null==t&&(t={},Object.assign(t,$t));for(let i in $t)$t.hasOwnProperty(i)&&t.hasOwnProperty(i)?t[i]in Kt?e[i]=Kt[t[i]]:ee('Material "'+t[i]+'" on wall "'+i+'" not found. Using "'+$t[i]+'".'):ee('Wall "'+i+'" is not defined. Default used.');return e}function le(t){null==t&&(t={});for(let e in Zt)t.hasOwnProperty(e)||(t[e]=Zt[e]);return t}function de(t,e,i){let n=new Float32Array(Jt);t=le(t),e=function(t){null==t&&(t={});for(let e in $t)t.hasOwnProperty(e)||(t[e]=Kt[$t[e]]);return t}(e),null==i&&(i=343);let s=55.2620422318571/i,r=t.width*t.height*t.depth;if(r<1e-4)return n;let o=t.width*t.height,a=t.width*t.depth,A=t.depth*t.height,c=2*(o+a+A);for(let t=0;t<Jt;t++){let i=((e.left[t]+e.right[t])*o+(e.down[t]+e.up[t])*a+(e.front[t]+e.back[t])*A)/c;n[t]=1.38*s*r/(-c*Math.log(1-i)+4*te[t]*r)}return n}function fe(t){let e=[];for(let i in Xt)if(Xt.hasOwnProperty(i)){e[i]=0;for(let n=0;n<3;n++){let s=n+4;e[i]+=t[i][s]}e[i]/=3,e[i]=Math.sqrt(1-e[i])}return e}class pe{constructor(t,e){null==e&&(e={}),null==e.listenerPosition&&(e.listenerPosition=Ut.slice()),null==e.dimensions&&(e.dimensions={},Object.assign(e.dimensions,Zt)),null==e.materials&&(e.materials={},Object.assign(e.materials,$t)),null==e.speedOfSound&&(e.speedOfSound=343),e.dimensions=le(e.dimensions);let i=ue(e.materials),n=fe(i),s=de(e.dimensions,i,e.speedOfSound);this.early=new he(t,{dimensions:e.dimensions,coefficients:n,speedOfSound:e.speedOfSound,listenerPosition:e.listenerPosition}),this.late=new ce(t,{durations:s}),this.speedOfSound=e.speedOfSound,this.output=t.createGain(),this.early.output.connect(this.output),this._merger=t.createChannelMerger(4),this.late.output.connect(this._merger,0,0),this._merger.connect(this.output)}dispose(){this.early.output.disconnect(this.output),this.late.output.disconnect(this._merger,0,0),this._merger.disconnect(this.output)}setProperties(t,e){let i=ue(e),n=de(t,i,this.speedOfSound);this.late.setDurations(n),this.early.speedOfSound=this.speedOfSound;let s=fe(i);this.early.setRoomProperties(t,s)}setListenerPosition(t,e,i){this.early.speedOfSound=this.speedOfSound,this.early.setListenerPosition(t,e,i);let n=this.getDistanceOutsideRoom(t,e,i),s=1;n>1e-8&&(s=1-n/1,s=Math.max(0,Math.min(1,s))),this.output.gain.value=s}getDistanceOutsideRoom(t,e,i){let n=Math.max(0,-this.early._halfDimensions.width-t,t-this.early._halfDimensions.width),s=Math.max(0,-this.early._halfDimensions.height-e,e-this.early._halfDimensions.height),r=Math.max(0,-this.early._halfDimensions.depth-i,i-this.early._halfDimensions.depth);return Math.sqrt(n*n+s*s+r*r)}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * limitations under the License.
 */
class ge{constructor(t,e){null==e&&(e={}),null==e.ambisonicOrder&&(e.ambisonicOrder=1),null==e.listenerPosition&&(e.listenerPosition=Ut.slice()),null==e.listenerForward&&(e.listenerForward=Gt.slice()),null==e.listenerUp&&(e.listenerUp=Yt.slice()),null==e.dimensions&&(e.dimensions={},Object.assign(e.dimensions,Zt)),null==e.materials&&(e.materials={},Object.assign(e.materials,$t)),null==e.speedOfSound&&(e.speedOfSound=343),null==e.renderingMode&&(e.renderingMode="ambisonic"),this._ambisonicOrder=se.validateAmbisonicOrder(e.ambisonicOrder),this._sources=[],this._room=new pe(t,{listenerPosition:e.listenerPosition,dimensions:e.dimensions,materials:e.materials,speedOfSound:e.speedOfSound}),this._listener=new re(t,{ambisonicOrder:e.ambisonicOrder,position:e.listenerPosition,forward:e.listenerForward,up:e.listenerUp,renderingMode:e.renderingMode}),this._context=t,this.output=t.createGain(),this.ambisonicOutput=t.createGain(),this.ambisonicInput=this._listener.input,this._room.output.connect(this._listener.input),this._listener.output.connect(this.output),this._listener.ambisonicOutput.connect(this.ambisonicOutput)}getRenderingMode(){return this._listener.getRenderingMode()}setRenderingMode(t){this._listener.setRenderingMode(t)}dispose(){this._room.output.disconnect(this._listener.input),this._listener.output.disconnect(this.output),this._listener.ambisonicOutput.disconnect(this.ambisonicOutput)}createSource(t){let e=new Ae(this,t);return this._sources[this._sources.length]=e,e}removeSource(t){const e=this._sources.findIndex(e=>e===t);e>-1&&(this._sources.splice(e,1),t.dispose())}setAmbisonicOrder(t){this._ambisonicOrder=se.validateAmbisonicOrder(t)}setRoomProperties(t,e){this._room.setProperties(t,e)}setListenerPosition(t,e,i){this._listener.position[0]=t,this._listener.position[1]=e,this._listener.position[2]=i,this._room.setListenerPosition(t,e,i),this._sources.forEach((function(t){t._update()}))}setListenerOrientation(t,e,i,n,s,r){this._listener.setOrientation(t,e,i,n,s,r)}setListenerFromMatrix(t){this._listener.setFromMatrix(t),this.setListenerPosition(this._listener.position[0],this._listener.position[1],this._listener.position[2])}setSpeedOfSound(t){this._room.speedOfSound=t}}ge.Version="2.0.0";class ve extends C{constructor(t,e,i,n,s){const r=s.createSource();super(t,e,i,n,r.input),this.resScene=s,this.resNode=r,Object.seal(this)}update(t){super.update(t);const{p:e,f:i,u:n}=t;this.resNode.setMinDistance(this.minDistance),this.resNode.setMaxDistance(this.maxDistance),this.resNode.setPosition(e.x,e.y,e.z),this.resNode.setOrientation(i.x,i.y,i.z,n.x,n.y,n.z)}dispose(){this.resScene.removeSource(this.resNode),this.resNode=null,super.dispose()}}class me extends I{constructor(t){super(),this.scene=new ge(t,{ambisonicOrder:3,renderingMode:"bypass"}),this.scene.output.connect(t.destination),this.scene.setRoomProperties({width:10,height:5,depth:10},{left:"transparent",right:"transparent",front:"transparent",back:"transparent",down:"grass",up:"transparent"}),Object.seal(this)}update(t){super.update(t);const{p:e,f:i,u:n}=t;this.scene.setListenerPosition(e.x,e.y,e.z),this.scene.setListenerOrientation(i.x,i.y,i.z,n.x,n.y,n.z)}createSource(t,e,i,n){return new ve(t,e,i,n,this.scene)}}const ye=new e;let we=Object.prototype.hasOwnProperty.call(window,"AudioContext"),_e=we&&Object.prototype.hasOwnProperty.call(window,"AudioListener"),xe=_e&&Object.prototype.hasOwnProperty.call(AudioListener.prototype,"setPosition"),De=_e&&Object.prototype.hasOwnProperty.call(AudioListener.prototype,"positionX"),Pe=_e;class be extends a{constructor(){super(),this.minDistance=1,this.minDistanceSq=1,this.maxDistance=10,this.maxDistanceSq=100,this.rolloff=1,this.transitionTime=.5,this.sources=new Map,this.clips=new Map,this.onAudioActivity=t=>{ye.id=t.id,ye.isActive=t.isActive,this.dispatchEvent(ye)},this.audioContext=null,this.listener=null,Object.seal(this)}start(){this.createContext()}update(){if(this.audioContext){const t=this.currentTime;for(let e of this.sources.values())e.update(t);for(let e of this.clips.values())e.update(t)}}createContext(){if(!this.audioContext){if(we)try{this.audioContext=new AudioContext}catch(t){we=!1,console.warn("Could not create WebAudio AudioContext",t)}if(we||(this.audioContext=new m),we&&Pe)try{this.listener=new me(this.audioContext)}catch(t){Pe=!1,console.warn("Resonance Audio API not available!",t)}if(we&&!Pe&&De)try{this.listener=new T(this.audioContext.listener)}catch(t){De=!1,console.warn("No AudioListener.positionX property!",t)}if(we&&!Pe&&!De&&xe)try{this.listener=new S(this.audioContext.listener)}catch(t){xe=!1,console.warn("No WebAudio API!",t)}xe&&we||(this.listener=new I)}}createSpatializer(t,e,i){if(!this.listener)throw new Error("Audio context isn't ready");if(!e)throw new Error("No stream or audio element given.");return this.listener.createSource(t,e,i,this.audioContext)}addClip(t,...e){const i=new v,n=e.map(t=>{const e=document.createElement("source");return e.src=t,e}),s=document.createElement("audio");s.controls=!1,s.playsInline=!0,s.append(...n),i.spatializer=this.createSpatializer(t,s),this.clips.set(t,i)}playClip(t,e=1){if(this.clips.has(t)){const i=this.clips.get(t);i.volume=e,i.spatializer.play()}}get currentTime(){return this.audioContext.currentTime}createSource(t){return this.sources.has(t)||this.sources.set(t,new v),this.sources.get(t)}getSource(t){return this.sources.get(t)||null}removeSource(t){if(this.sources.has(t)){const e=this.sources.get(t);this.sources.delete(t),e.dispose()}}setSourceStream(t,e){if(this.sources.has(t)){const i=this.sources.get(t);i.spatializer&&(i.spatializer.removeEventListener("audioActivity",this.onAudioActivity),i.spatializer=null),e&&(i.spatializer=this.createSpatializer(t,e,1024),i.spatializer&&(i.spatializer.audio&&(i.spatializer.audio.autoPlay=!0,i.spatializer.audio.muted=!0,i.spatializer.audio.addEventListener("onloadedmetadata",()=>i.spatializer.audio.play()),i.spatializer.audio.play()),i.spatializer.setAudioProperties(this.minDistance,this.maxDistance,this.rolloff,this.transitionTime),i.spatializer.addEventListener("audioActivity",this.onAudioActivity)))}}setAudioProperties(t,e,i,n){this.minDistance=t,this.maxDistance=e,this.transitionTime=n,this.rolloff=i;for(let t of this.sources.values())t.spatializer&&t.spatializer.setAudioProperties(this.minDistance,this.maxDistance,this.rolloff,this.transitionTime);for(let t of this.clips.values())t.spatializer&&t.spatializer.setAudioProperties(this.minDistance,this.maxDistance,this.rolloff,this.transitionTime)}setUserPosition(t,e,i,n){if(this.sources.has(t)){this.sources.get(t).pose.setTargetPosition(e,i,n,this.currentTime,this.transitionTime)}}setUserOrientation(t,e,i,n,s,r,o){if(this.sources.has(t)){this.sources.get(t).pose.setTargetOrientation(e,i,n,s,r,o,this.currentTime,this.transitionTime)}}setUserPose(t,e,i,n,s,r,o,a,A,c){if(this.sources.has(t)){this.sources.get(t).pose.setTarget(e,i,n,s,r,o,a,A,c,this.currentTime,this.transitionTime)}}}function Ce(t,e){return i=>{t(i),e(i)}}function Ee(t,e,i,n){return void 0===n&&o(i)&&(n=i,i=void 0),new Promise((o,a)=>{const A=s(e);if(A){const i=o,n=()=>{t.removeEventListener(e,i)};o=Ce(n,o),a=Ce(n,a)}const c=s(i);if(c){const e=a,n=()=>{t.removeEventListener(i,e)};o=Ce(n,o),a=Ce(n,a)}if(r(n)){const t=setTimeout(a,n,`'${e}' has timed out.`),i=()=>clearTimeout(t);o=Ce(i,o),a=Ce(i,a)}A&&t.addEventListener(e,o),c&&t.addEventListener(i,()=>{a("Rejection event found")})})}
/*!
