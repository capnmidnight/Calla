 ...
 */function p(){const e=`[Omnitone] ${Array.prototype.slice.call(arguments).join(" ")} (${performance.now().toFixed(2)}ms)`;window.console.log(e)}function g(){const e=`[Omnitone] ${Array.prototype.slice.call(arguments).join(" ")} (${performance.now().toFixed(2)}ms)`;throw window.console.error(e),new Error(e)}let Gt,qt,Yt,Xt,Vt,Zt,Jt,Kt,$t,ei,ti,ii,ni,si,ai,oi,ri,li,ci,ui,hi,Ai,di,pi,gi,fi,mi,vi,wi;function f(e){return e instanceof AudioContext||e instanceof OfflineAudioContext}
/**
 * @license
 ...
 * limitations under the License.
 */
const yi={BASE64:"base64",URL:"url"};class Di{constructor(e,t,i){f(e)||g("BufferList: Invalid BaseAudioContext."),this._context=e,this._options={dataType:yi.BASE64,verbose:!1},i&&(i.dataType&&function(e,t){for(let i in e)if(t===e[i])return!0;return!1}(yi,i.dataType)&&(this._options.dataType=i.dataType),i.verbose&&(this._options.verbose=Boolean(i.verbose))),this._bufferData=this._options.dataType===yi.BASE64?t:t.slice(0)}async load(){try{const e=this._bufferData.map((e,t)=>this._launchAsyncLoadTask(e,t)),t=await Promise.all(e);return p("BufferList: "+(this._options.dataType===yi.BASE64?this._bufferData.length+" AudioBuffers from Base64-encoded HRIRs":this._bufferData.length+" files via XHR")+" loaded successfully."),t}catch(e){g('BufferList: error while loading "'+bData+'". ('+e.message+")")}}async _launchAsyncLoadTask(e,t){const i=await this._fetch(e),n=await this._context.decodeAudioData(i);return p("BufferList: "+(this._options.dataType===yi.BASE64?"ArrayBuffer("+t+") from Base64-encoded HRIR":'"'+e+'"')+" successfully loaded."),n}async _fetch(e){if(this._options.dataType===yi.BASE64)return function(e){const t=window.atob(e),i=new Uint8Array(t.length);return i.forEach((e,n)=>i[n]=t.charCodeAt(n)),i.buffer}(e);{const t=await fetch(e);return await t.arrayBuffer()}}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Bi={DEFAULT:[0,1,2,3],SAFARI:[2,0,1,3],FUMA:[0,3,1,2]};class Pi{constructor(e,t){this._context=e,this._splitter=this._context.createChannelSplitter(4),this._merger=this._context.createChannelMerger(4),this.input=this._splitter,this.output=this._merger,this.setChannelMap(t||Bi.DEFAULT)}setChannelMap(e){Array.isArray(e)&&(this._channelMap=e,this._splitter.disconnect(),this._splitter.connect(this._merger,0,this._channelMap[0]),this._splitter.connect(this._merger,1,this._channelMap[1]),this._splitter.connect(this._merger,2,this._channelMap[2]),this._splitter.connect(this._merger,3,this._channelMap[3]))}dipose(){this._splitter.disconnect(this._merger,0,this._channelMap[0]),this._splitter.disconnect(this._merger,1,this._channelMap[1]),this._splitter.disconnect(this._merger,2,this._channelMap[2]),this._splitter.disconnect(this._merger,3,this._channelMap[3])}}Pi.ChannelMap=Bi;
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * limitations under the License.
 */
const Ei={AMBISONIC:"ambisonic",BYPASS:"bypass",OFF:"off"};class _i{constructor(e,t){f(e)||g("FOARenderer: Invalid BaseAudioContext."),this._context=e,this._config={channelMap:Pi.ChannelMap.DEFAULT,renderingMode:Ei.AMBISONIC},t&&(t.channelMap&&(Array.isArray(t.channelMap)&&4===t.channelMap.length?this._config.channelMap=t.channelMap:g("FOARenderer: Invalid channel map. (got "+t.channelMap+")")),t.hrirPathList&&(Array.isArray(t.hrirPathList)&&2===t.hrirPathList.length?this._config.pathList=t.hrirPathList:g("FOARenderer: Invalid HRIR URLs. It must be an array with 2 URLs to HRIR files. (got "+t.hrirPathList+")")),t.renderingMode&&(Object.values(Ei).includes(t.renderingMode)?this._config.renderingMode=t.renderingMode:p("FOARenderer: Invalid rendering mode order. (got"+t.renderingMode+') Fallbacks to the mode "ambisonic".'))),this._buildAudioGraph(),this._tempMatrix4=new Float32Array(16)}_buildAudioGraph(){this.input=this._context.createGain(),this.output=this._context.createGain(),this._bypass=this._context.createGain(),this._foaRouter=new Pi(this._context,this._config.channelMap),this._foaRotator=new Ci(this._context),this._foaConvolver=new xi(this._context),this.input.connect(this._foaRouter.input),this.input.connect(this._bypass),this._foaRouter.output.connect(this._foaRotator.input),this._foaRotator.output.connect(this._foaConvolver.input),this._foaConvolver.output.connect(this.output),this.input.channelCount=4,this.input.channelCountMode="explicit",this.input.channelInterpretation="discrete"}dipose(){mode===Ei.BYPASS&&this._bypass.connect(this.output),this.input.disconnect(this._foaRouter.input),this.input.disconnect(this._bypass),this._foaRouter.output.disconnect(this._foaRotator.input),this._foaRotator.output.disconnect(this._foaConvolver.input),this._foaConvolver.output.disconnect(this.output),this._foaConvolver.dispose(),this._foaRotator.dispose(),this._foaRouter.dipose()}async initialize(){p("FOARenderer: Initializing... (mode: "+this._config.renderingMode+")");const e=this._config.pathList?new Di(this._context,this._config.pathList,{dataType:"url"}):new Di(this._context,bi);try{const t=await e.load();this._foaConvolver.setHRIRBufferList(t),this.setRenderingMode(this._config.renderingMode),p("FOARenderer: HRIRs loaded successfully. Ready.")}catch(e){g("FOARenderer: HRIR loading/decoding failed. Reason: "+e.message)}}setChannelMap(e){e.toString()!==this._config.channelMap.toString()&&(p("Remapping channels (["+this._config.channelMap.toString()+"] -> ["+e.toString()+"])."),this._config.channelMap=e.slice(),this._foaRouter.setChannelMap(this._config.channelMap))}setRotationMatrix3(e){this._foaRotator.setRotationMatrix3(e)}setRotationMatrix4(e){this._foaRotator.setRotationMatrix4(e)}setRotationMatrixFromCamera(e){var t,i;t=this._tempMatrix4,i=e.elements,Gt=i[0],qt=i[1],Yt=i[2],Xt=i[3],Vt=i[4],Zt=i[5],Jt=i[6],Kt=i[7],$t=i[8],ei=i[9],ti=i[10],ii=i[11],ni=i[12],si=i[13],ai=i[14],oi=i[15],ri=Gt*Zt-qt*Vt,li=Gt*Jt-Yt*Vt,ci=Gt*Kt-Xt*Vt,ui=qt*Jt-Yt*Zt,hi=qt*Kt-Xt*Zt,Ai=Yt*Kt-Xt*Jt,di=$t*si-ei*ni,pi=$t*ai-ti*ni,gi=$t*oi-ii*ni,fi=ei*ai-ti*si,mi=ei*oi-ii*si,vi=ti*oi-ii*ai,wi=ri*vi-li*mi+ci*fi+ui*gi-hi*pi+Ai*di,wi&&(wi=1/wi,t[0]=(Zt*vi-Jt*mi+Kt*fi)*wi,t[1]=(Yt*mi-qt*vi-Xt*fi)*wi,t[2]=(si*Ai-ai*hi+oi*ui)*wi,t[3]=(ti*hi-ei*Ai-ii*ui)*wi,t[4]=(Jt*gi-Vt*vi-Kt*pi)*wi,t[5]=(Gt*vi-Yt*gi+Xt*pi)*wi,t[6]=(ai*ci-ni*Ai-oi*li)*wi,t[7]=($t*Ai-ti*ci+ii*li)*wi,t[8]=(Vt*mi-Zt*gi+Kt*di)*wi,t[9]=(qt*gi-Gt*mi-Xt*di)*wi,t[10]=(ni*hi-si*ci+oi*ri)*wi,t[11]=(ei*ci-$t*hi-ii*ri)*wi,t[12]=(Zt*pi-Vt*fi-Jt*di)*wi,t[13]=(Gt*fi-qt*pi+Yt*di)*wi,t[14]=(si*li-ni*ui-ai*ri)*wi,t[15]=($t*ui-ei*li+ti*ri)*wi),this._foaRotator.setRotationMatrix4(this._tempMatrix4)}setRenderingMode(e){if(e!==this._config.renderingMode){switch(e){case Ei.AMBISONIC:this._foaConvolver.enable(),this._bypass.disconnect();break;case Ei.BYPASS:this._foaConvolver.disable(),this._bypass.connect(this.output);break;case Ei.OFF:this._foaConvolver.disable(),this._bypass.disconnect();break;default:return void p('FOARenderer: Rendering mode "'+e+'" is not supported.')}this._config.renderingMode=e,p("FOARenderer: Rendering mode changed. ("+e+")")}}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * limitations under the License.
 */
class On{constructor(e,t){null==t&&(t={}),null==t.ambisonicOrder&&(t.ambisonicOrder=nn),null==t.azimuth&&(t.azimuth=en),null==t.elevation&&(t.elevation=tn),null==t.sourceWidth&&(t.sourceWidth=sn),this._context=e,this.input=e.createGain(),this._channelGain=[],this._merger=void 0,this.output=e.createGain(),this.setAmbisonicOrder(t.ambisonicOrder),this._azimuth=t.azimuth,this._elevation=t.elevation,this.setSourceWidth(t.sourceWidth)}setAmbisonicOrder(e){this._ambisonicOrder=On.validateAmbisonicOrder(e),this.input.disconnect();for(let e=0;e<this._channelGain.length;e++)this._channelGain[e].disconnect();null!=this._merger&&this._merger.disconnect(),delete this._channelGain,delete this._merger;let t=(this._ambisonicOrder+1)*(this._ambisonicOrder+1);this._merger=this._context.createChannelMerger(t),this._channelGain=new Array(t);for(let e=0;e<t;e++)this._channelGain[e]=this._context.createGain(),this.input.connect(this._channelGain[e]),this._channelGain[e].connect(this._merger,0,e);this._merger.connect(this.output)}dispose(){this._merger.disconnect(this.output);let e=(this._ambisonicOrder+1)*(this._ambisonicOrder+1);for(let t=0;t<e;++t)this._channelGain[t].disconnect(this._merger,0,t),this.input.disconnect(this._channelGain[t])}setDirection(e,t){(null==e||isNaN(e))&&(e=en),(null==t||isNaN(t))&&(t=tn),this._azimuth=e,this._elevation=t,(e=Math.round(e%360))<0&&(e+=360),t=Math.round(Math.min(90,Math.max(-90,t)))+90,this._channelGain[0].gain.value=Qi[this._spreadIndex][0];for(let i=1;i<=this._ambisonicOrder;i++){let n=Qi[this._spreadIndex][i];for(let s=-i;s<=i;s++){let a=i*i+i+s,o=i*(i+1)/2+Math.abs(s)-1,r=Ni[1][t][o];if(0!=s){let t=Hi+s-1;s<0&&(t=Hi+s),r*=Ni[0][e][t]}this._channelGain[a].gain.value=r*n}}}setSourceWidth(e){this._spreadIndex=Math.min(359,Math.max(0,Math.round(e))),this.setDirection(this._azimuth,this._elevation)}}On.validateAmbisonicOrder=function(e){return isNaN(e)||null==e?(Tn("Error: Invalid ambisonic order",options.ambisonicOrder,"\nUsing ambisonicOrder=1 instead."),e=1):e<1?(Tn("Error: Unable to render ambisonic order",options.ambisonicOrder,"(Min order is 1)","\nUsing min order instead."),e=1):e>Hi&&(Tn("Error: Unable to render ambisonic order",options.ambisonicOrder,"(Max order is",Hi,")\nUsing max order instead."),options.ambisonicOrder=Hi),e};
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Nn{constructor(e,t){null==t&&(t={}),null==t.alpha&&(t.alpha=Ki),null==t.sharpness&&(t.sharpness=$i),this._context=e,this._lowpass=e.createBiquadFilter(),this._lowpass.type="lowpass",this._lowpass.Q.value=0,this._lowpass.frequency.value=.5*e.sampleRate,this._cosTheta=0,this.setPattern(t.alpha,t.sharpness),this.input=this._lowpass,this.output=this._lowpass}computeAngle(e,t){let i=In(e),n=In(t),s=1;if(this._alpha>kn){let e=i[0]*n[0]+i[1]*n[1]+i[2]*n[2];s=1-this._alpha+this._alpha*e,s=Math.pow(Math.abs(s),this._sharpness)}this._lowpass.frequency.value=.5*this._context.sampleRate*s}setPattern(e,t){this._alpha=Math.min(1,Math.max(0,e)),this._sharpness=Math.max(1,t),this.computeAngle([this._cosTheta*this._cosTheta,0,0],[1,0,0])}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Hn{constructor(e,t){null==t&&(t={}),null==t.minDistance&&(t.minDistance=Zi),null==t.maxDistance&&(t.maxDistance=Ji),null==t.rolloff&&(t.rolloff=Vi),this.minDistance=t.minDistance,this.maxDistance=t.maxDistance,this.setRolloff(t.rolloff),this._gainNode=e.createGain(),this.setDistance(t.maxDistance),this.input=this._gainNode,this.output=this._gainNode}setDistance(e){let t=1;if("logarithmic"==this._rolloff){if(e>this.maxDistance)t=0;else if(e>this.minDistance){let i=this.maxDistance-this.minDistance;if(i>kn){let n=1/(i+1);t=(1/(e-this.minDistance+1)-n)/(1-n)}}}else if("linear"==this._rolloff)if(e>this.maxDistance)t=0;else if(e>this.minDistance){let i=this.maxDistance-this.minDistance;i>kn&&(t=(this.maxDistance-e)/i)}this._gainNode.gain.value=t}setRolloff(e){let t=~Xi.indexOf(e);null!=e&&t?e=e.toString().toLowerCase():(t||Tn('Invalid rolloff model ("'+e+'"). Using default: "'+Vi+'".'),e=Vi),this._rolloff=e}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Qn{constructor(e,t){null==t&&(t={}),null==t.position&&(t.position=Wi.slice()),null==t.forward&&(t.forward=Gi.slice()),null==t.up&&(t.up=qi.slice()),null==t.minDistance&&(t.minDistance=Zi),null==t.maxDistance&&(t.maxDistance=Ji),null==t.rolloff&&(t.rolloff=Vi),null==t.gain&&(t.gain=zi),null==t.alpha&&(t.alpha=Ki),null==t.sharpness&&(t.sharpness=$i),null==t.sourceWidth&&(t.sourceWidth=sn),this._scene=e,this._position=t.position,this._forward=t.forward,this._up=t.up,this._dx=new Float32Array(3),this._right=Ln(this._forward,this._up);let i=e._context;this.input=i.createGain(),this._directivity=new Nn(i,{alpha:t.alpha,sharpness:t.sharpness}),this._toEarly=i.createGain(),this._toLate=i.createGain(),this._attenuation=new Hn(i,{minDistance:t.minDistance,maxDistance:t.maxDistance,rolloff:t.rolloff}),this._encoder=new On(i,{ambisonicOrder:e._ambisonicOrder,sourceWidth:t.sourceWidth}),this.input.connect(this._toLate),this._toLate.connect(e._room.late.input),this.input.connect(this._attenuation.input),this._attenuation.output.connect(this._toEarly),this._toEarly.connect(e._room.early.input),this._attenuation.output.connect(this._directivity.input),this._directivity.output.connect(this._encoder.input),this._encoder.output.connect(e._listener.input),this.setPosition(t.position[0],t.position[1],t.position[2]),this.input.gain.value=t.gain}dispose(){this._encoder.output.disconnect(this._scene._listener.input),this._directivity.output.disconnect(this._encoder.input),this._attenuation.output.disconnect(this._directivity.input),this._toEarly.disconnect(this._scene._room.early.input),this._attenuation.output.disconnect(this._toEarly),this.input.disconnect(this._attenuation.input),this._toLate.disconnect(this._scene._room.late.input),this.input.disconnect(this._toLate),this._encoder.dispose()}setPosition(e,t,i){this._position[0]=e,this._position[1]=t,this._position[2]=i;let n=function(e){let t=1;return e>kn&&(t=1-e/Ui,t=Math.max(0,Math.min(1,t))),t}(this._scene._room.getDistanceOutsideRoom(this._position[0],this._position[1],this._position[2]));this._toLate.gain.value=n,this._toEarly.gain.value=n,this._update()}_update(){for(let e=0;e<3;e++)this._dx[e]=this._position[e]-this._scene._listener.position[e];let e=Math.sqrt(this._dx[0]*this._dx[0]+this._dx[1]*this._dx[1]+this._dx[2]*this._dx[2]);e>0&&(this._dx[0]/=e,this._dx[1]/=e,this._dx[2]/=e);let t=Math.atan2(-this._dx[0],this._dx[2])*Fn,i=Math.atan2(this._dx[1],Math.sqrt(this._dx[0]*this._dx[0]+this._dx[2]*this._dx[2]))*Fn;this._attenuation.setDistance(e),this._directivity.computeAngle(this._forward,this._dx),this._encoder.setDirection(t,i)}setRolloff(e){this._attenuation.setRolloff(e)}setMinDistance(e){this._attenuation.minDistance=e}setMaxDistance(e){this._attenuation.maxDistance=e}setGain(e){this.input.gain.value=e}setOrientation(e,t,i,n,s,a){this._forward[0]=e,this._forward[1]=t,this._forward[2]=i,this._up[0]=n,this._up[1]=s,this._up[2]=a,this._right=Ln(this._forward,this._up)}setFromMatrix(e){this._right[0]=e.elements[0],this._right[1]=e.elements[1],this._right[2]=e.elements[2],this._up[0]=e.elements[4],this._up[1]=e.elements[5],this._up[2]=e.elements[6],this._forward[0]=e.elements[8],this._forward[1]=e.elements[9],this._forward[2]=e.elements[10],this._right=In(this._right),this._up=In(this._up),this._forward=In(this._forward),this.setPosition(e.elements[12],e.elements[13],e.elements[14])}setSourceWidth(e){this._encoder.setSourceWidth(e),this.setPosition(this._position[0],this._position[1],this._position[2])}setDirectivityPattern(e,t){this._directivity.setPattern(e,t),this.setPosition(this._position[0],this._position[1],this._position[2])}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class zn{constructor(e,t){null==t&&(t={}),null==t.durations&&(t.durations=wn.slice()),null==t.predelay&&(t.predelay=dn),null==t.gain&&(t.gain=gn),null==t.bandwidth&&(t.bandwidth=hn),null==t.tailonset&&(t.tailonset=pn);let i=t.predelay/1e3;this._bandwidthCoeff=t.bandwidth*Mn,this._tailonsetSamples=t.tailonset/1e3,this._context=e,this.input=e.createGain(),this._predelay=e.createDelay(i),this._convolver=e.createConvolver(),this.output=e.createGain(),this.output.gain.value=t.gain,this._convolver.normalize=!1,this.input.connect(this._predelay),this._predelay.connect(this._convolver),this._convolver.connect(this.output),this.setDurations(t.durations)}dispose(){this.input.disconnect(this._predelay),this._predelay.disconnect(this._convolver),this._convolver.disconnect(this.output)}setDurations(e){if(e.length!==vn)return void Tn("Warning: invalid number of RT60 values provided to reverb.");let t=new Float32Array(vn),i=this._context.sampleRate;for(let n=0;n<e.length;n++)e[n]=Math.max(0,Math.min(fn,e[n])),t[n]=Math.round(e[n]*i*An);let n=0;for(let e=0;e<t.length;e++)t[e]>n&&(n=t[e]);n<1&&(n=1);let s=this._context.createBuffer(1,n,i),a=s.getChannelData(0),o=new Float32Array(n);for(let e=0;e<n;e++)o[e]=2*Math.random()-1;for(let e=0;e<vn;e++){let n=-Sn/t[e],s=En*mn[e]/i,r=Math.sin(s),l=r*Math.sinh(this._bandwidthCoeff*s/r),c=1/(1+l),u=l*c,h=-2*Math.cos(s)*c,A=(1-l)*c,d=0,p=0;for(let i=0;i<t[e];i++){let e=o[i]*Math.exp(n*i)-h*d-A*p;a[i]+=u*(e-p),p=d,d=e}}let r=Math.round(this._tailonsetSamples);for(let e=0;e<Math.min(a.length,r);e++){let t=.5*(1-Math.cos(En*e/(2*r-1)));a[e]*=t}this._convolver.buffer=s}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class jn{constructor(e,t){null==t&&(t={}),null==t.speedOfSound&&(t.speedOfSound=Yi),null==t.listenerPosition&&(t.listenerPosition=Wi.slice()),null==t.coefficients&&(t.coefficients={},Object.assign(t.coefficients,rn)),this.speedOfSound=t.speedOfSound,this.input=e.createGain(),this.output=e.createGain(),this._lowpass=e.createBiquadFilter(),this._delays={},this._gains={},this._inverters={},this._merger=e.createChannelMerger(4);for(let t in rn)rn.hasOwnProperty(t)&&(this._delays[t]=e.createDelay(an),this._gains[t]=e.createGain());this._inverters.right=e.createGain(),this._inverters.down=e.createGain(),this._inverters.back=e.createGain(),this._lowpass.type="lowpass",this._lowpass.frequency.value=on,this._lowpass.Q.value=0;for(let e in rn)rn.hasOwnProperty(e)&&(this._delays[e].delayTime.value=0,this._gains[e].gain.value=0);this._inverters.right.gain.value=-1,this._inverters.down.gain.value=-1,this._inverters.back.gain.value=-1,this.input.connect(this._lowpass);for(let e in rn)rn.hasOwnProperty(e)&&(this._lowpass.connect(this._delays[e]),this._delays[e].connect(this._gains[e]),this._gains[e].connect(this._merger,0,0));this._gains.left.connect(this._merger,0,1),this._gains.right.connect(this._inverters.right),this._inverters.right.connect(this._merger,0,1),this._gains.up.connect(this._merger,0,2),this._gains.down.connect(this._inverters.down),this._inverters.down.connect(this._merger,0,2),this._gains.front.connect(this._merger,0,3),this._gains.back.connect(this._inverters.back),this._inverters.back.connect(this._merger,0,3),this._merger.connect(this.output),this._listenerPosition=t.listenerPosition,this.setRoomProperties(t.dimensions,t.coefficients)}dipose(){this.input.disconnect(this._lowpass);for(let e in rn)rn.hasOwnProperty(e)&&(this._lowpass.disconnect(this._delays[e]),this._delays[e].disconnect(this._gains[e]),this._gains[e].disconnect(this._merger,0,0));this._gains.left.disconnect(this._merger,0,1),this._gains.right.disconnect(this._inverters.right),this._inverters.right.disconnect(this._merger,0,1),this._gains.up.disconnect(this._merger,0,2),this._gains.down.disconnect(this._inverters.down),this._inverters.down.disconnect(this._merger,0,2),this._gains.front.disconnect(this._merger,0,3),this._gains.back.disconnect(this._inverters.back),this._inverters.back.disconnect(this._merger,0,3),this._merger.disconnect(this.output)}setListenerPosition(e,t,i){this._listenerPosition=[e,t,i];let n={left:un*Math.max(0,this._halfDimensions.width+e)+ln,right:un*Math.max(0,this._halfDimensions.width-e)+ln,front:un*Math.max(0,this._halfDimensions.depth+i)+ln,back:un*Math.max(0,this._halfDimensions.depth-i)+ln,down:un*Math.max(0,this._halfDimensions.height+t)+ln,up:un*Math.max(0,this._halfDimensions.height-t)+ln};for(let e in rn)if(rn.hasOwnProperty(e)){let t=n[e]/this.speedOfSound;this._delays[e].delayTime.value=t;let i=this._coefficients[e]/n[e];this._gains[e].gain.value=i}}setRoomProperties(e,t){null==e&&(e={},Object.assign(e,cn)),null==t&&(t={},Object.assign(t,rn)),this._coefficients=t,this._halfDimensions={},this._halfDimensions.width=.5*e.width,this._halfDimensions.height=.5*e.height,this._halfDimensions.depth=.5*e.depth,this.setListenerPosition(this._listenerPosition[0],this._listenerPosition[1],this._listenerPosition[2])}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function S(e){let t={};for(let e in Dn)Dn.hasOwnProperty(e)&&(t[e]=yn[Dn[e]]);null==e&&(e={},Object.assign(e,Dn));for(let i in Dn)Dn.hasOwnProperty(i)&&e.hasOwnProperty(i)?e[i]in yn?t[i]=yn[e[i]]:Tn('Material "'+e[i]+'" on wall "'+i+'" not found. Using "'+Dn[i]+'".'):Tn('Wall "'+i+'" is not defined. Default used.');return t}function M(e){null==e&&(e={});for(let t in cn)e.hasOwnProperty(t)||(e[t]=cn[t]);return e}function F(e,t,i){let n=new Float32Array(vn);e=M(e),t=function(e){null==e&&(e={});for(let t in Dn)e.hasOwnProperty(t)||(e[t]=yn[Dn[t]]);return e}(t),null==i&&(i=Yi);let s=_n/i,a=e.width*e.height*e.depth;if(a<Cn)return n;let o=e.width*e.height,r=e.width*e.depth,l=e.depth*e.height,c=2*(o+r+l);for(let e=0;e<vn;e++){let i=((t.left[e]+t.right[e])*o+(t.down[e]+t.up[e])*r+(t.front[e]+t.back[e])*l)/c;n[e]=Pn*s*a/(-c*Math.log(1-i)+4*Bn[e]*a)}return n}function k(e){let t=[];for(let i in rn)if(rn.hasOwnProperty(i)){t[i]=0;for(let n=0;n<xn;n++){let s=n+bn;t[i]+=e[i][s]}t[i]/=xn,t[i]=Math.sqrt(1-t[i])}return t}class Un{constructor(e,t){null==t&&(t={}),null==t.listenerPosition&&(t.listenerPosition=Wi.slice()),null==t.dimensions&&(t.dimensions={},Object.assign(t.dimensions,cn)),null==t.materials&&(t.materials={},Object.assign(t.materials,Dn)),null==t.speedOfSound&&(t.speedOfSound=Yi),t.dimensions=M(t.dimensions);let i=S(t.materials),n=k(i),s=F(t.dimensions,i,t.speedOfSound);this.early=new jn(e,{dimensions:t.dimensions,coefficients:n,speedOfSound:t.speedOfSound,listenerPosition:t.listenerPosition}),this.late=new zn(e,{durations:s}),this.speedOfSound=t.speedOfSound,this.output=e.createGain(),this.early.output.connect(this.output),this._merger=e.createChannelMerger(4),this.late.output.connect(this._merger,0,0),this._merger.connect(this.output)}dispose(){this.early.output.disconnect(this.output),this.late.output.disconnect(this._merger,0,0),this._merger.disconnect(this.output)}setProperties(e,t){let i=S(t),n=F(e,i,this.speedOfSound);this.late.setDurations(n),this.early.speedOfSound=this.speedOfSound;let s=k(i);this.early.setRoomProperties(e,s)}setListenerPosition(e,t,i){this.early.speedOfSound=this.speedOfSound,this.early.setListenerPosition(e,t,i);let n=this.getDistanceOutsideRoom(e,t,i),s=1;n>kn&&(s=1-n/ji,s=Math.max(0,Math.min(1,s))),this.output.gain.value=s}getDistanceOutsideRoom(e,t,i){let n=Math.max(0,-this.early._halfDimensions.width-e,e-this.early._halfDimensions.width),s=Math.max(0,-this.early._halfDimensions.height-t,t-this.early._halfDimensions.height),a=Math.max(0,-this.early._halfDimensions.depth-i,i-this.early._halfDimensions.depth);return Math.sqrt(n*n+s*s+a*a)}}
/**
 * @license
 * Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * limitations under the License.
 */
class Wn{constructor(e,t){null==t&&(t={}),null==t.ambisonicOrder&&(t.ambisonicOrder=nn),null==t.listenerPosition&&(t.listenerPosition=Wi.slice()),null==t.listenerForward&&(t.listenerForward=Gi.slice()),null==t.listenerUp&&(t.listenerUp=qi.slice()),null==t.dimensions&&(t.dimensions={},Object.assign(t.dimensions,cn)),null==t.materials&&(t.materials={},Object.assign(t.materials,Dn)),null==t.speedOfSound&&(t.speedOfSound=Yi),this._ambisonicOrder=On.validateAmbisonicOrder(t.ambisonicOrder),this._sources=[],this._room=new Un(e,{listenerPosition:t.listenerPosition,dimensions:t.dimensions,materials:t.materials,speedOfSound:t.speedOfSound}),this._listener=new Rn(e,{ambisonicOrder:t.ambisonicOrder,position:t.listenerPosition,forward:t.listenerForward,up:t.listenerUp}),this._context=e,this.output=e.createGain(),this.ambisonicOutput=e.createGain(),this.ambisonicInput=this._listener.input,this._room.output.connect(this._listener.input),this._listener.output.connect(this.output),this._listener.ambisonicOutput.connect(this.ambisonicOutput)}dispose(){this._room.output.disconnect(this._listener.input),this._listener.output.disconnect(this.output),this._listener.ambisonicOutput.disconnect(this.ambisonicOutput)}createSource(e){let t=new Qn(this,e);return this._sources[this._sources.length]=t,t}removeSource(e){const t=this._sources.findIndex(t=>t===e);t>-1&&(this._sources.splice(t,1),e.dispose())}setAmbisonicOrder(e){this._ambisonicOrder=On.validateAmbisonicOrder(e)}setRoomProperties(e,t){this._room.setProperties(e,t)}setListenerPosition(e,t,i){this._listener.position[0]=e,this._listener.position[1]=t,this._listener.position[2]=i,this._room.setListenerPosition(e,t,i),this._sources.forEach((function(e){e._update()}))}setListenerOrientation(e,t,i,n,s,a){this._listener.setOrientation(e,t,i,n,s,a)}setListenerFromMatrix(e){this._listener.setFromMatrix(e),this.setListenerPosition(this._listener.position[0],this._listener.position[1],this._listener.position[2])}setSpeedOfSound(e){this._room.speedOfSound=e}}Wn.Version="2.0.0";class Gn extends Ot{constructor(e,t,i,n,s){const a=s.createSource();super(e,t,i,n,a.input),this.resScene=s,this.resNode=a,Object.seal(this)}update(e){super.update(e);const{p:t,f:i,u:n}=e;this.resNode.setMinDistance(this.minDistance),this.resNode.setMaxDistance(this.maxDistance),this.resNode.setPosition(t.x,t.y,t.z),this.resNode.setOrientation(i.x,i.y,i.z,n.x,n.y,n.z)}dispose(){this.resScene.removeSource(this.resNode),this.resNode=null,super.dispose()}}class qn extends Qt{constructor(e){super(),this.scene=new Wn(e,{ambisonicOrder:3}),this.scene.output.connect(e.destination),this.scene.setRoomProperties({width:10,height:5,depth:10},{left:"transparent",right:"transparent",front:"transparent",back:"transparent",down:"grass",up:"transparent"}),Object.seal(this)}update(e){super.update(e);const{p:t,f:i,u:n}=e;this.scene.setListenerPosition(t.x,t.y,t.z),this.scene.setListenerOrientation(i.x,i.y,i.z,n.x,n.y,n.z)}createSource(e,t,i,n){return new Gn(e,t,i,n,this.scene)}}const Yn=1024,Xn=new xt;let Vn=Object.prototype.hasOwnProperty.call(window,"AudioContext"),Zn=Vn&&Object.prototype.hasOwnProperty.call(window,"AudioListener"),Jn=Zn&&Object.prototype.hasOwnProperty.call(AudioListener.prototype,"setPosition"),Kn=Zn&&Object.prototype.hasOwnProperty.call(AudioListener.prototype,"positionX"),$n=Zn;class es extends bt{constructor(){super(),this.minDistance=1,this.minDistanceSq=1,this.maxDistance=10,this.maxDistanceSq=100,this.rolloff=1,this.transitionTime=.5,this.sources=new Map,this.clips=new Map,this.onAudioActivity=e=>{Xn.id=e.id,Xn.isActive=e.isActive,this.dispatchEvent(Xn)},this.audioContext=null,this.listener=null,Object.seal(this)}start(){this.createContext()}update(){if(this.audioContext){const e=this.currentTime;for(let t of this.sources.values())t.update(e);for(let t of this.clips.values())t.update(e)}}createContext(){if(!this.audioContext){if(Vn)try{this.audioContext=new AudioContext}catch(e){Vn=!1,console.warn("Could not create WebAudio AudioContext",e)}if(Vn||(this.audioContext=new St),Vn&&$n)try{this.listener=new qn(this.audioContext)}catch(e){$n=!1,console.warn("Resonance Audio API not available!",e)}if(Vn&&!$n&&Kn)try{this.listener=new jt(this.audioContext.listener)}catch(e){Kn=!1,console.warn("No AudioListener.positionX property!",e)}if(Vn&&!$n&&!Kn&&Jn)try{this.listener=new Wt(this.audioContext.listener)}catch(e){Jn=!1,console.warn("No WebAudio API!",e)}Jn&&Vn||(this.listener=new Qt)}}createSpatializer(e,t,i){if(!this.listener)throw new Error("Audio context isn't ready");if(!t)throw new Error("No stream or audio element given.");return this.listener.createSource(e,t,i,this.audioContext)}addClip(e,...t){const i=new _t,n=t.map(e=>{const t=document.createElement("source");return t.src=e,t}),s=document.createElement("audio");s.controls=!1,s.playsInline=!0,s.append(...n),i.spatializer=this.createSpatializer(e,s),this.clips.set(e,i)}playClip(e,t=1){if(this.clips.has(e)){const i=this.clips.get(e);i.volume=t,i.spatializer.play()}}get currentTime(){return this.audioContext.currentTime}createSource(e){return this.sources.has(e)||this.sources.set(e,new _t),this.sources.get(e)}getSource(e){return this.sources.get(e)||null}removeSource(e){if(this.sources.has(e)){const t=this.sources.get(e);this.sources.delete(e),t.dispose()}}setSourceStream(e,t){if(this.sources.has(e)){const i=this.sources.get(e);i.spatializer&&(i.spatializer.removeEventListener("audioActivity",this.onAudioActivity),i.spatializer=null),t&&(i.spatializer=this.createSpatializer(e,t,Yn),i.spatializer&&(i.spatializer.audio&&(i.spatializer.audio.autoPlay=!0,i.spatializer.audio.muted=!0,i.spatializer.audio.addEventListener("onloadedmetadata",()=>i.spatializer.audio.play()),i.spatializer.audio.play()),i.spatializer.setAudioProperties(this.minDistance,this.maxDistance,this.rolloff,this.transitionTime),i.spatializer.addEventListener("audioActivity",this.onAudioActivity)))}}setAudioProperties(e,t,i,n){this.minDistance=e,this.maxDistance=t,this.transitionTime=n,this.rolloff=i;for(let e of this.sources.values())e.spatializer&&e.spatializer.setAudioProperties(this.minDistance,this.maxDistance,this.rolloff,this.transitionTime);for(let e of this.clips.values())e.spatializer&&e.spatializer.setAudioProperties(this.minDistance,this.maxDistance,this.rolloff,this.transitionTime)}setUserPosition(e,t,i,n){if(this.sources.has(e)){this.sources.get(e).pose.setTargetPosition(t,i,n,this.currentTime,this.transitionTime)}}setUserOrientation(e,t,i,n,s,a,o){if(this.sources.has(e)){this.sources.get(e).pose.setTargetOrientation(t,i,n,s,a,o,this.currentTime,this.transitionTime)}}setUserPose(e,t,i,n,s,a,o,r,l,c){if(this.sources.has(e)){this.sources.get(e).pose.setTarget(t,i,n,s,a,o,r,l,c,this.currentTime,this.transitionTime)}}}function T(e,t){return i=>{e(i),t(i)}}function I(e,t){for(let i in t){let n=t[i],s=void 0;n instanceof Array&&(s=n[1],n=n[0]),e.addEventListener(i,n,s)}}function L(e,t,i,n){return void 0===n&&l(i)&&(n=i,i=void 0),new Promise((s,r)=>{const l=a(t);if(l){const i=s,n=()=>{e.removeEventListener(t,i)};s=T(n,s),r=T(n,r)}const c=a(i);if(c){const t=r,n=()=>{e.removeEventListener(i,t)};s=T(n,s),r=T(n,r)}if(o(n)){const e=setTimeout(r,n,`'${t}' has timed out.`),i=()=>clearTimeout(e);s=T(i,s),r=T(i,r)}l&&e.addEventListener(t,s),c&&e.addEventListener(i,()=>{r("Rejection event found")})})}
/*!
