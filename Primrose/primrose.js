...
export class Primrose extends TypedEventBase {
    constructor(options) {
        ...
        const fillRect = (gfx, fill, x, y, w, h) => {
            gfx.fillStyle = fill;
            gfx.fillRect(x * character.width, y * character.height, w * character.width + 1, h * character.height + 1);
        };
        const strokeRect = (gfx, stroke, x, y, w, h) => {
            gfx.strokeStyle = stroke;
            gfx.strokeRect(x * character.width, y * character.height, w * character.width + 1, h * character.height + 1);
        };
        const renderCanvasBackground = () => {
            const minCursor = Cursor.min(frontCursor, backCursor), maxCursor = Cursor.max(frontCursor, backCursor);
            ...
            bgfx.translate((gridBounds.x - scroll.x) * character.width + pad, -scroll.y * character.height + pad);
            ...
            for (let y = minY; y <= maxY && y < rows.length; ++y) {
                ...
                for (let i = 0; i < row.length; ++i) {
                    ...
                    if (minX <= tokenBack.x && tokenFront.x <= maxX) {
                        ...
                        if (inSelection) {
                            const selectionFront = Cursor.max(minCursor, tokenFront), selectionBack = Cursor.min(maxCursor, tokenBack), cw = selectionBack.i - selectionFront.i;
        ...
        const renderCanvasForeground = () => {
            ...
            fgfx.translate((gridBounds.x - scroll.x) * character.width + pad, pad);
            const minY = scroll.y | 0, maxY = minY + gridBounds.height, minX = scroll.x | 0, maxX = minX + gridBounds.width;
            tokenFront.setXY(rows, 0, minY);
            tokenBack.copy(tokenFront);
            for (let y = minY; y <= maxY && y < rows.length; ++y) {
                ...
                for (let i = 0; i < row.length; ++i) {
                    ...
                    if (minX <= tokenBack.x
                        ...
                        fgfx.fillText(t.value, tokenFront.x * character.width, textY);
                    }
                    tokenFront.copy(tokenBack);
                }
                tokenFront.x = 0;
        ...
        const renderCanvasTrim = () => {
            ...
            if (showLineNumbers) {
                fillRect(tgfx, theme.selectedBackColor ||
                    DefaultTheme.selectedBackColor, 0, 0, gridBounds.x, this.width - pad * 2);
                strokeRect(tgfx, theme.regular.foreColor ||
                    DefaultTheme.regular.foreColor, 0, 0, gridBounds.x, this.height - pad * 2);
            }
            let maxRowWidth = 2;
            tgfx.save();
            {
                ...
                for (let y = minY; y <= maxY && y < rows.length; ++y) {
                    ...
                    if (showLineNumbers) {
                        ...
                        if (row.lineNumber > lastLineNumber) {
                            ...
                            tgfx.fillText(row.lineNumber.toFixed(0), 0, y * character.height);
                        }
                    }
                }
            }
            ...
            if (showScrollBars) {
                ...
                if (!wordWrap && maxRowWidth > gridBounds.width) {
                    const drawWidth = gridBounds.width * character.width - pad, scrollX = (scroll.x * drawWidth) / maxRowWidth + gridBounds.x * character.width, scrollBarWidth = drawWidth * (gridBounds.width / maxRowWidth), by = this.height - character.height - pad, bw = Math.max(character.width, scrollBarWidth);
                ...
                if (rows.length > gridBounds.height) {
                    const drawHeight = gridBounds.height * character.height, scrollY = (scroll.y * drawHeight) / rows.length, scrollBarHeight = drawHeight * (gridBounds.height / rows.length), bx = this.width - vScrollWidth * character.width - 2 * pad, bw = vScrollWidth * character.width, bh = Math.max(character.height, scrollBarHeight);
        ...
        const doRender = () => {
            if (theme) {
                const textChanged = lastText !== value, focusChanged = focused !== lastFocused, fontChanged = context.font !== lastFont, paddingChanged = pad !== lastPadding, themeChanged = theme.name !== lastThemeName, boundsChanged = gridBounds.toString() !== lastGridBounds, characterWidthChanged = character.width !== lastCharacterWidth, characterHeightChanged = character.height !== lastCharacterHeight, cursorChanged = frontCursor.i !== lastFrontCursor
                    || backCursor.i !== lastBackCursor, scrollChanged = scroll.x !== lastScrollX
                    || scroll.y !== lastScrollY, layoutChanged = resized
                    || boundsChanged
                    || textChanged
        ...
        const setSelectedText = (txt) => {
            ...
            if (frontCursor.i !== backCursor.i || txt.length > 0) {
                const minCursor = Cursor.min(frontCursor, backCursor), maxCursor = Cursor.max(frontCursor, backCursor), startRow = rows[minCursor.y], endRow = rows[maxCursor.y], unchangedLeft = value.substring(0, startRow.startStringIndex), unchangedRight = value.substring(endRow.endStringIndex), changedStartSubStringIndex = minCursor.i - startRow.startStringIndex, changedLeft = startRow.substring(0, changedStartSubStringIndex), changedEndSubStringIndex = maxCursor.i - endRow.startStringIndex, changedRight = endRow.substring(changedEndSubStringIndex), changedText = changedLeft + txt + changedRight;
        ...
        const refreshTokens = (startY, endY, txt) => {
            ...
            const newTokens = language.tokenize(txt), startRow = rows[startY], startTokenIndex = startRow.startTokenIndex, startLineNumber = startRow.lineNumber, startStringIndex = startRow.startStringIndex, endRow = rows[endY], endTokenIndex = endRow.endTokenIndex, tokenRemoveCount = endTokenIndex - startTokenIndex, oldTokens = tokens.splice(startTokenIndex, tokenRemoveCount, ...newTokens);
            ...
            // measure the grid
            const x = Math.floor(lineCountWidth + pad / character.width), y = Math.floor(pad / character.height), w = Math.floor((this.width - 2 * pad) / character.width) - x - bottomRightGutter.width, h = Math.floor((this.height - 2 * pad) / character.height) - y - bottomRightGutter.height;
            ...
            for (let i = 0; i < tokenQueue.length; ++i) {
                const t = tokenQueue[i], widthLeft = gridBounds.width - currentString.length, wrap = wordWrap && t.type !== FinalTokenType.newLines && t.length > widthLeft, breakLine = t.type === FinalTokenType.newLines || wrap;
                if (wrap) {
        ...
        const clampScroll = () => {
            const toHigh = scroll.y < 0 || maxVerticalScroll === 0, toLow = scroll.y > maxVerticalScroll;
            if (toHigh) {
                scroll.y = 0;
            }
            else if (toLow) {
        ...
        const scrollIntoView = (currentCursor) => {
            const dx = minDelta(currentCursor.x, scroll.x, scroll.x + gridBounds.width), dy = minDelta(currentCursor.y, scroll.y, scroll.y + gridBounds.height);
            this.scrollBy(dx, dy);
        };
        const pushUndo = () => {
        ...
        const keyDownCommands = new Map([
            ...
            ["RemoveTab", () => {
                    const row = rows[frontCursor.y], toDelete = Math.min(frontCursor.x, tabWidth);
        ...
        const setTouchPointer = (touch) => {
            if (isHTMLCanvas(canv)) {
                ...
                pointer.set(touch.clientX - cb.left, touch.clientY - cb.top);
            }
        };
        ...
        const setUVPointer = (evt) => {
            pointer.set(evt.uv.x * this.width, (1 - evt.uv.y) * this.height);
        };
            ...
            fontSize: {
                ...
                set: (s) => {
                    ...
                    if (s !== fontSize) {
                        ...
                        // measureText.
                        character.width = context.measureText("MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM")
                            .width /
                            100;
                        refreshAllTokens();
                    }
        ...
        let value = "", pad = 0, theme = DefaultTheme, tabWidth = 2, canv = null, resized = false, hovered = false, focused = false, fontSize = null, scaleFactor = 2, tabString = "  ", readOnly = false, wordWrap = false, historyIndex = -1, multiLine = false, tabPressed = false, lineCount = 1, lineCountWidth = 0, element = null, language = JavaScript, showScrollBars = false, showLineNumbers = false, controlType = singleLineOutput, maxVerticalScroll = 0, lastCharacterHeight = null, lastCharacterWidth = null, lastFrontCursor = null, lastGridBounds = null, lastBackCursor = null, lastThemeName = null, lastPadding = null, lastFocused = null, lastScrollX = null, lastScrollY = null, lastFont = null, lastText = null;
        const history = new Array(), tokens = new Array(), rows = [Row.emptyRow(0, 0, 0)], scroll = new Point(), pointer = new Point(), character = new Rectangle(), bottomRightGutter = new Size(), gridBounds = new Rectangle(), tokenBack = new Cursor(), tokenFront = new Cursor(), backCursor = new Cursor(), frontCursor = new Cursor(), os = isApple ? MacOS : Windows;
        //<<<<<<<<<< PRIVATE MUTABLE FIELDS <<<<<<<<<<
        //>>>>>>>>>> SETUP CANVAS >>>>>>>>>>
        ...
        if (options.element === null) {
            ...
            canv = Canvas(styles(width("100%"), height("100%")));
        ...
        //<<<<<<<<<< SETUP CANVAS <<<<<<<<<<
        //>>>>>>>>>> SETUP BUFFERS >>>>>>>>>>
        const context = canv.getContext("2d"), fg = createUtilityCanvas(canv.width, canv.height), fgfx = fg.getContext("2d"), bg = createUtilityCanvas(canv.width, canv.height), bgfx = bg.getContext("2d"), tg = createUtilityCanvas(canv.width, canv.height), tgfx = tg.getContext("2d");
