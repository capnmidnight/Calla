...
import type { MaterialCoefficients } from "./MaterialCoefficients";
 ...
 */
function _getCoefficientsFromMaterials(materials: RoomMaterials) {
    // Initialize coefficients to use defaults.
    const coefficients: MaterialCoefficients = {
        [Direction.Left]: null,
 ...
 * @return {Object}
 */
function _sanitizeCoefficients(coefficients: MaterialCoefficients) {
 ...
 */
function _sanitizeDimensions(dimensions: RoomDimensions): RoomDimensions {
 ...
 */
function _getDurationsFromProperties(dimensions: RoomDimensions, coefficients: MaterialCoefficients, speedOfSound: number): Float32Array {
 ...
 * @return {Object}
 */
function _computeReflectionCoefficients(absorptionCoefficients: MaterialCoefficients) {
    const reflectionCoefficients: ReflectionCoefficients = {
        [Direction.Left]: 0,
        [Direction.Right]: 0,
        [Direction.Front]: 0,
        [Direction.Back]: 0,
...
export class Room implements IDisposable {
    ...
    constructor(context: BaseAudioContext, options?: RoomOptions) {
        ...
        const durations = _getDurationsFromProperties(options.dimensions,
    ...
    setProperties(dimensions: RoomDimensions, materials: RoomMaterials): void {
        ...
        const durations = _getDurationsFromProperties(dimensions,
    ...
    getDistanceOutsideRoom(v: vec3): number {
        const dx = Math.max(
            0,
            -this.early.halfDimensions.width - v[0],
            v[0] - this.early.halfDimensions.width);

        const dy = Math.max(
            0,
            -this.early.halfDimensions.height - v[1],
            v[1] - this.early.halfDimensions.height);

        const dz = Math.max(
            0,
            -this.early.halfDimensions.depth - v[2],
            v[2] - this.early.halfDimensions.depth);
